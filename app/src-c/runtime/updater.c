...
char* ms_update_check_json(void) {
    ...
    char* result;
    if (text == NULL)
        return strdup("{\"ok\":false,\"error\":\"failed to fetch release\",\"current_version\":\"0.65.5\"}");
    release = ms_json_parse(text, strlen(text), error, sizeof(error));
    free(text);
    if (release == NULL)
        return strdup("{\"ok\":false,\"error\":\"failed to parse release\",\"current_version\":\"0.65.5\"}");
