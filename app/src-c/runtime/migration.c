...
#define MIGRATION_VERSION "0.72.0"
...
char* ms_migration_progress_json(const char* home) {
    ...
    if (!p)
        return NULL;
    out = raw_or(p,
                 "{\"status\":\"idle\",\"step\":0,\"total\":0,\"message\":\"\",\"error\":null,\"version\":\"0.72.0\"}");
    free(p);
    return out;
}
char* ms_migration_report_json(const char* home) {
    ...
    if (!p)
        return NULL;
    out = raw_or(p, "{\"schema_version\":1,\"status\":\"idle\",\"version\":\"0.72.0\",\"entries\":[],\"summary\":\"No "
