...
function registerIpc() {
  ...
  ipcMain.handle("app:run-steam-fix", async () => {
    return new Promise((resolve) => {
      const { spawn } = require("child_process");
      const env = { ...process.env, PATH: ensureShellPath() };
      // Runs the Steam CEF webhelper wrapper deployment script verbatim via bash stdin.
      const proc = spawn("/bin/bash", [], { env });
      let output = "";
      ...
      proc.on("close", (code: number | null) => {
        resolve(
          code === 0
            ...
            : { ok: false, error: output.trim() || `Steam fix exited with code ${code}` },
        );
      });
      proc.stdin.end(STEAM_FIX_SCRIPT);
    });
  });
