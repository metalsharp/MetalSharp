...
- **DXMT launch family**: M9/M10/M11/VKD3D to Metal
...
|---|---|
| **VKD3D** | D3D12 -> vkd3d-proton -> Vulkan -> MoltenVK -> Metal (DXMT rollback via `vkd3dBackend=dxmt`) |
| **M11** | D3D11 -> DXMT -> Metal |
| **M10** | D3D10 -> DXMT -> Metal |
| **M9** | D3D9 -> MetalSharp D3D9 -> DXMT launch family -> Metal |

`dxmt` is the internal auto-router that selects VKD3D/M11/M10/M9. M32, Wine, and macOS Steam are backend fallback/diagnostic paths, not normal graphics-route buttons.

## DXMT

The DXMT launch family is used by M11, M10, and M9, and by VKD3D only when the
`vkd3dBackend=dxmt` rollback is selected.
...
|---|---|
| `d3d12.dll` | VKD3D |
| `d3d11.dll` | VKD3D, M11, M10 |
| `dxgi.dll` | VKD3D, M11, M10 |
| `d3d10core.dll` | VKD3D, M11, M10 |
| `d3d10.dll`, `d3d10_1.dll` | M10 public Wine D3D10 entrypoints |
| `winemetal.dll` | VKD3D, M11, M10 |
| `d3d9.dll` | M9 |
...
routes prefer the explicit M9/M10/M11/VKD3D cache namespaces.
...
| Subnautica | M11 |
| Subnautica: Below Zero | VKD3D recommended, M11 optimized |
| Rain World | M11, M9 also works |
| Undertale | M9 |
| Portal 2 | M9 |
| Nidhogg 2 | M9 |
| Ghostrunner | M11; VKD3D only if needed |
...
- VKD3D is the primary stable DXMT D3D engine method in source.
