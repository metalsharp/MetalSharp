...
| SER/HitObject operations | `262–289` — `HitObject_TraceRay` through `HitObject_Attributes` | 28 | Open; the required HitObject/SER behavior and exact readbacks are not implemented. Focused native ray-generation and inline-RayQuery evidence does not substitute for these rows. The descending pass starts at row 280 and has continued through row 279: minimal pinned `lib_6_9` shaders containing `HitObject_WorldToObject3x4` and `HitObject_ObjectToWorld3x4` reach both DXIL lowerers and are rejected with `unsupported_intrinsics=5`, `unsupported_opcodes=0`, including three explicit diagnostics for each opcode; this is fail-closed negative evidence only. |
...
negative, not a DXIL runtime proof. With a freshly rebuilt ABI-matched runtime,
the direct DXIL probe records `unsupported_intrinsics=1`,
...
creation. CycleCounterLegacy still has no exact runtime readback. The remaining opcode work now proceeds in the requested descending order,
beginning with row 280 and continuing toward row 234. Rows 280 and 279 have
only fail-closed negative evidence; each must be followed by exact positive
behavior or an explicitly validated semantically equivalent provider before
promotion.
