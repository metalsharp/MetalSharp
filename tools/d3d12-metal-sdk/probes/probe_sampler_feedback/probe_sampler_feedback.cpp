...
void main(uint3 id : SV_DispatchThreadID) {
  ...
  auto serialize = proc<SerializeRootSignatureFn>(
  ...
  if (!write_file(shader_path, shader))
    ...
    dxc_exit = run_process(
        "dxc.exe -nologo -T cs_6_5 -E main -HV 2021 -Od "
    ...
    if (dxc_exit != 0 || dxil.empty())
      ...
      hr = create_device(nullptr, D3D_FEATURE_LEVEL_12_1,
    ...
    if (SUCCEEDED(hr)) {
      ...
      for (DXGI_FORMAT format : formats) {
        ...
        HRESULT support_hr = device->CheckFeatureSupport(
            D3D12_FEATURE_FORMAT_SUPPORT, &support, sizeof(support));
        format_support_ok &= SUCCEEDED(support_hr) &&
                             (support.Support1 &
    ...
    if (SUCCEEDED(hr))
      hr = serialize(&root_desc, D3D_ROOT_SIGNATURE_VERSION_1_0, &root_blob,
                     &root_errors);
    if (SUCCEEDED(hr))
    ...
    if (SUCCEEDED(hr)) {
      ...
      for (uint32_t i = 0; i < 2 && SUCCEEDED(hr); ++i) {
        ...
        if (feedback_resource[i]) {
          ...
          logical_desc_ok &= actual.Width == 64 && actual.Height == 64 &&
                             actual.MipLevels == 4 &&
      ...
      feedback_created = SUCCEEDED(hr) && feedback_resource[0] &&
                         feedback_resource[1] && feedback_resource[2] &&
      ...
      HRESULT invalid_hr = device8->CreateCommittedResource2(
          &default_heap, D3D12_HEAP_FLAG_NONE, &invalid_desc,
          D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, nullptr,
          IID_PPV_ARGS(&invalid_resource));
      invalid_feedback_desc_rejected =
          invalid_hr == E_INVALIDARG && invalid_resource == nullptr;
    ...
    if (SUCCEEDED(hr)) {
      ...
      hr = device->CreateCommittedResource(
          &readback_heap, D3D12_HEAP_FLAG_NONE, &desc,
    ...
    if (SUCCEEDED(hr)) {
      ...
      for (uint32_t i = 0; i < 3; ++i) {
        device8->CreateSamplerFeedbackUnorderedAccessView(
            paired, feedback_resource[i], cpu);
        cpu.ptr += stride;
      }
      device8->CreateSamplerFeedbackUnorderedAccessView(
          paired_array_resource, feedback_resource[4], cpu);
      cpu.ptr += stride;
    ...
    if (SUCCEEDED(hr)) {
      ...
      const UINT clear_min[4] = {UINT32_MAX, UINT32_MAX, UINT32_MAX,
                                 UINT32_MAX};
      const UINT clear_used[4] = {};
      D3D12_CPU_DESCRIPTOR_HANDLE feedback_cpu[4] = {
          resource_heap->GetCPUDescriptorHandleForHeapStart(),
          resource_heap->GetCPUDescriptorHandleForHeapStart(),
          resource_heap->GetCPUDescriptorHandleForHeapStart(),
          resource_heap->GetCPUDescriptorHandleForHeapStart()};
      ...
      for (uint32_t i = 0; i < 3; ++i) {
        ...
        list->ClearUnorderedAccessViewUint(
            feedback_gpu, feedback_cpu[i], feedback_resource[i],
      ...
      list->ClearUnorderedAccessViewUint(
          array_feedback_gpu, feedback_cpu[3], feedback_resource[4], clear_min,
          0, nullptr);
      list->Dispatch(1, 1, 1);
      for (uint32_t i = 0; i < 3; ++i) {
        uav_barrier(i);
        transition(feedback_resource[i],
                   D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                   D3D12_RESOURCE_STATE_RESOLVE_SOURCE);
      }
      uav_barrier(4);
      transition(feedback_resource[4],
                 D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                 D3D12_RESOURCE_STATE_RESOLVE_SOURCE);
      for (uint32_t i = 0; i < 2; ++i)
        list1->ResolveSubresourceRegion(
            decoded[i], 0, 0, 0, feedback_resource[i], 0, nullptr,
            DXGI_FORMAT_R8_UINT,
            D3D12_RESOLVE_MODE_DECODE_SAMPLER_FEEDBACK);
      for (uint32_t mip = 0; mip < 4; ++mip)
        list1->ResolveSubresourceRegion(
            used_decoded[mip], 0, 0, 0, feedback_resource[2], mip, nullptr,
            DXGI_FORMAT_R8_UINT,
            D3D12_RESOLVE_MODE_DECODE_SAMPLER_FEEDBACK);
      list1->ResolveSubresourceRegion(
          array_decoded[0], 0, 0, 0, feedback_resource[4], 0, nullptr,
          DXGI_FORMAT_R8_UINT,
          D3D12_RESOLVE_MODE_DECODE_SAMPLER_FEEDBACK);
      list1->ResolveSubresourceRegion(
          array_decoded[1], 0, 0, 0, feedback_resource[4], 4, nullptr,
          DXGI_FORMAT_R8_UINT,
          D3D12_RESOLVE_MODE_DECODE_SAMPLER_FEEDBACK);
      transition(decoded[0], D3D12_RESOURCE_STATE_RESOLVE_DEST,
                 D3D12_RESOURCE_STATE_RESOLVE_SOURCE);
      list1->ResolveSubresourceRegion(
          feedback_resource[3], 0, 0, 0, decoded[0], 0, nullptr,
          DXGI_FORMAT_R8_UINT,
          D3D12_RESOLVE_MODE_ENCODE_SAMPLER_FEEDBACK);
      transition(feedback_resource[3], D3D12_RESOURCE_STATE_RESOLVE_DEST,
                 D3D12_RESOURCE_STATE_RESOLVE_SOURCE);
      list1->ResolveSubresourceRegion(
          encoded_decoded, 0, 0, 0, feedback_resource[3], 0, nullptr,
          DXGI_FORMAT_R8_UINT,
          D3D12_RESOLVE_MODE_DECODE_SAMPLER_FEEDBACK);
      transition(decoded[0], D3D12_RESOURCE_STATE_RESOLVE_SOURCE,
                 D3D12_RESOURCE_STATE_COPY_SOURCE);
      transition(decoded[1], D3D12_RESOURCE_STATE_RESOLVE_DEST,
                 D3D12_RESOURCE_STATE_COPY_SOURCE);
      for (uint32_t i = 0; i < 4; ++i)
        transition(used_decoded[i], D3D12_RESOURCE_STATE_RESOLVE_DEST,
                   D3D12_RESOURCE_STATE_COPY_SOURCE);
      transition(encoded_decoded, D3D12_RESOURCE_STATE_RESOLVE_DEST,
                 D3D12_RESOURCE_STATE_COPY_SOURCE);
      for (uint32_t i = 0; i < 2; ++i)
        transition(array_decoded[i], D3D12_RESOURCE_STATE_RESOLVE_DEST,
                   D3D12_RESOURCE_STATE_COPY_SOURCE);

      auto copy_to_readback = [&](ID3D12Resource *source, UINT width,
                                  UINT height, UINT64 offset) {
      ...
      for (uint32_t mip = 0; mip < 4; ++mip)
        copy_to_readback(used_decoded[mip], used_sizes[mip],
                         used_sizes[mip], used_offsets[mip]);
    ...
    if (SUCCEEDED(hr) && mapped) {
      ...
      for (uint32_t slice = 0; slice < 2; ++slice) {
        for (uint32_t y = 0; y < 4; ++y) {
          for (uint32_t x = 0; x < 4; ++x) {
            observed[slice][y][x] =
                mapped[slice * 1024 + y * 256 + x];
            const uint8_t expected =
                slice == 0 ? static_cast<uint8_t>((x + 2 * y) & 3)
      ...
      for (uint32_t y = 0; y < 4; ++y) {
        for (uint32_t x = 0; x < 4; ++x) {
          ...
          contention_min_ok &= observed[1][y][x] ==
                               ((x == 0 && y == 0) ? 0 : 255);
        }
      }
      const UINT used_sizes[4] = {4, 2, 1, 1};
      ...
      for (uint32_t y = 0; y < 4; ++y) {
        for (uint32_t x = 0; x < 4; ++x) {
          ...
          for (uint32_t layer = 0; layer < 2; ++layer) {
            const uint8_t array_expected =
                static_cast<uint8_t>((x + 2 * y + layer) & 3);
            const uint8_t array_value =
                mapped[5120 + layer * 1024 + y * 256 + x];
  ...
  HRESULT options_hr = device
                           ? device->CheckFeatureSupport(
                                 D3D12_FEATURE_D3D12_OPTIONS7, &options7,
                                 sizeof(options7))
                           : E_FAIL;
  const bool passed = SUCCEEDED(hr) && dxc_exit == 0 && device8_ok &&
                      format_support_ok && feedback_created &&
                      placed_feedback_created &&
                      invalid_feedback_desc_rejected && logical_desc_ok &&
                      pso_created &&
                      unique_map_ok && contention_min_ok &&
                      mip_region_used_ok && encode_roundtrip_ok &&
