...
static D3D12_RESOURCE_DESC texture_desc(UINT64 width, UINT height,
                                        DXGI_FORMAT format, UINT16 mips,
...
float4 ps_level(VSOut input) : SV_Target0 {
  ...
  HRESULT hr = E_FAIL;
  DWORD vs_dxc = UINT32_MAX, ps_dxc[4] = {UINT32_MAX, UINT32_MAX,
  ...
  if (!write_file(shader_path, source))
    ...
    vs_dxc = run_process(
        "dxc.exe -nologo -T vs_6_0 -E vs_main -HV 2021 -Od "
        "-Fo Z:\\tmp\\metalsharp_sampler_feedback_vs.cso "
        "Z:\\tmp\\metalsharp_sampler_feedback_pixel.hlsl");
    const char *ps_entries[4] = {"ps_implicit", "ps_bias", "ps_grad",
                                 "ps_level"};
    ...
    for (uint32_t i = 0; i < 4; ++i) {
      ...
      std::snprintf(path, sizeof(path),
                    "Z:\\tmp\\metalsharp_sampler_feedback_ps%u.cso", i);
      ps[i] = read_file(path);
    }
    ...
    if (SUCCEEDED(hr)) {
      auto desc = texture_desc(64, 64, DXGI_FORMAT_R8G8B8A8_UNORM, 4,
    ...
    for (uint32_t i = 0; i < 4 && SUCCEEDED(hr); ++i) {
      auto desc = texture_desc(1, 1, DXGI_FORMAT_R8_UINT, 1,
                               D3D12_RESOURCE_FLAG_NONE);
      hr = device->CreateCommittedResource(
          &default_heap, D3D12_HEAP_FLAG_NONE, &desc,
    ...
    if (SUCCEEDED(hr)) {
      auto desc = texture_desc(4, 4, DXGI_FORMAT_R8G8B8A8_UNORM, 1,
    ...
    if (SUCCEEDED(hr))
      hr = create_heap(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 5,
                       D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE,
                       &resource_heap);
    if (SUCCEEDED(hr))
      hr = create_heap(D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER, 1,
                       D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE,
                       &sampler_heap);
    if (SUCCEEDED(hr))
      hr = create_heap(D3D12_DESCRIPTOR_HEAP_TYPE_RTV, 1,
    ...
    if (SUCCEEDED(hr)) {
      ...
      for (uint32_t i = 0; i < 4; ++i) {
        device8->CreateSamplerFeedbackUnorderedAccessView(paired, feedback[i],
                                                          cpu);
        cpu.ptr += stride;
      }
      ...
      device->CreateRenderTargetView(
          color_target, &rtv, rtv_heap->GetCPUDescriptorHandleForHeapStart());
    }
    ...
    if (SUCCEEDED(hr)) {
      ...
      input_elements[0] = {"POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0,
                           D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0};
      input_elements[1] = {"TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 12,
                           D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0};
      ...
      for (uint32_t i = 0; i < 4 && SUCCEEDED(hr); ++i) {
        ...
        hr = device->CreateGraphicsPipelineState(&desc,
                                                 IID_PPV_ARGS(&pso[i]));
        pso_ok &= SUCCEEDED(hr);
      }
    }
    ...
    if (SUCCEEDED(hr))
      hr = device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT,
                                     allocator, pso[0], IID_PPV_ARGS(&list));
    if (SUCCEEDED(hr))
    ...
    if (SUCCEEDED(hr)) {
      ...
      for (uint32_t i = 0; i < 4; ++i) {
        list->ClearUnorderedAccessViewUint(gpu, cpu, feedback[i], clear, 0,
                                           nullptr);
        gpu.ptr += stride;
        cpu.ptr += stride;
      }
      ...
      for (uint32_t i = 0; i < 4; ++i) {
        ...
        list1->ResolveSubresourceRegion(
            decoded[i], 0, 0, 0, feedback[i], 0, nullptr,
            DXGI_FORMAT_R8_UINT,
  ...
  if (SUCCEEDED(hr) && readback) {
    ...
    if (SUCCEEDED(hr) && mapped) {
      ...
      values_ok = observed[0] == 3 && observed[1] == 2 &&
                  observed[2] == 0 && observed[3] == 2;
  ...
  for (uint32_t i = 0; i < 4; ++i)
    dxc_ok &= ps_dxc[i] == 0;
  const bool passed = SUCCEEDED(hr) && dxc_ok && pso_ok && values_ok &&
                      color_ok;
  std::printf("{\n");
  ...
  std::printf("  \"ps_dxc_exit_codes\": [%lu, %lu, %lu, %lu],\n",
              ...
              static_cast<unsigned long>(ps_dxc[1]),
              static_cast<unsigned long>(ps_dxc[2]),
