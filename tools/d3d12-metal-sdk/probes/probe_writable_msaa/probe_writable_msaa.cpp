...
static float half_to_float(uint16_t value) {
    ...
    if (exponent == 0) {
        if (fraction == 0) {
            ...
            bits = sign | ((127u - 14u - shift) << 23) | ((normalized & 0x3ffu) << 13);
        }
...
static HRESULT execute_and_wait(ID3D12Device* device, ID3D12CommandQueue* queue, ID3D12GraphicsCommandList* list) {
    ...
    if (SUCCEEDED(result) && WaitForSingleObject(event_handle, 15000) != WAIT_OBJECT_0)
...
float4 graphics_ps(GraphicsVSOut input, uint sample_index : SV_SampleIndex,
                   uint coverage : SV_Coverage) : SV_Target0 {
  float2 sample_position = GetRenderTargetSamplePosition(0);
  float2 texture_sample_position = target.GetSamplePosition(0);
  uint sample_count = GetRenderTargetSampleCount();
  float4 centroid_value = EvaluateAttributeCentroid(input.color);
  float4 sample = EvaluateAttributeAtSample(input.color, 0);
  float4 snapped_value = EvaluateAttributeSnapped(input.color, int2(0, 0));
  target.sample[sample_index][uint2(0,0)] =
      float4(300.0 + sample_position.x + sample_position.y +
                 texture_sample_position.x + texture_sample_position.y +
                 sample_count + (float)sample_index + (float)coverage,
             0.0, 0.0, 1.0);
  ...
  return float4(centroid_value.x, sample.y, snapped_value.z, 1.0);
}
)";
    ...
    const DWORD dxc_store = run_process("dxc.exe -nologo -T cs_6_7 -E store -HV 2021 -Fo "
    ...
    auto serialize_root = load_proc<D3D12SerializeRootSignatureFn>(d3d12, "D3D12SerializeRootSignature");
    ...
    HRESULT root_serialize_hr =
        serialize_root && device ? serialize_root(&root_desc, D3D_ROOT_SIGNATURE_VERSION_1_0, &root_blob, &root_errors)
                                 : E_FAIL;
    if (root_errors)
        root_errors->Release();

    ID3D12RootSignature* root = nullptr;
    HRESULT root_create_hr = SUCCEEDED(root_serialize_hr) && root_blob
    ...
    HRESULT store_pso_hr = device && root && !store_dxil.empty()
                               ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&store_pso))
                               : E_FAIL;
    pipeline_desc.CS.pShaderBytecode = load_dxil.data();
    pipeline_desc.CS.BytecodeLength = load_dxil.size();
    HRESULT load_pso_hr = device && root && !load_dxil.empty()
                              ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&load_pso))
                              : E_FAIL;
    pipeline_desc.CS.pShaderBytecode = store_2_dxil.data();
    pipeline_desc.CS.BytecodeLength = store_2_dxil.size();
    HRESULT store_2_pso_hr = device && root && !store_2_dxil.empty()
                                 ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&store_2_pso))
                                 : E_FAIL;
    pipeline_desc.CS.pShaderBytecode = load_2_dxil.data();
    pipeline_desc.CS.BytecodeLength = load_2_dxil.size();
    HRESULT load_2_pso_hr = device && root && !load_2_dxil.empty()
                                ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&load_2_pso))
                                : E_FAIL;
    pipeline_desc.CS.pShaderBytecode = store_8_dxil.data();
    pipeline_desc.CS.BytecodeLength = store_8_dxil.size();
    HRESULT store_8_pso_hr = device && root && !store_8_dxil.empty()
                                 ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&store_8_pso))
                                 : E_FAIL;
    pipeline_desc.CS.pShaderBytecode = load_8_dxil.data();
    pipeline_desc.CS.BytecodeLength = load_8_dxil.size();
    HRESULT load_8_pso_hr = device && root && !load_8_dxil.empty()
                                ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&load_8_pso))
                                : E_FAIL;
    pipeline_desc.CS.pShaderBytecode = store_r8_dxil.data();
    pipeline_desc.CS.BytecodeLength = store_r8_dxil.size();
    HRESULT store_r8_pso_hr = device && root && !store_r8_dxil.empty()
                                  ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&store_r8_pso))
                                  : E_FAIL;
    pipeline_desc.CS.pShaderBytecode = load_r8_dxil.data();
    pipeline_desc.CS.BytecodeLength = load_r8_dxil.size();
    HRESULT load_r8_pso_hr = device && root && !load_r8_dxil.empty()
                                 ? device->CreateComputePipelineState(&pipeline_desc, IID_PPV_ARGS(&load_r8_pso))
                                 : E_FAIL;
    ...
    graphics_pipeline_desc.BlendState.RenderTarget[0].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
    ...
    graphics_pipeline_desc.DepthStencilState.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
    graphics_pipeline_desc.DepthStencilState.DepthFunc = D3D12_COMPARISON_FUNC_ALWAYS;
    ...
    HRESULT heap_hr = SUCCEEDED(list_hr) ? device->CreateDescriptorHeap(&heap_desc, IID_PPV_ARGS(&heap)) : E_FAIL;
    ...
    HRESULT target_hr =
        SUCCEEDED(heap_hr)
            ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &target_desc,
                                              D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&target))
            : E_FAIL;
    D3D12_RESOURCE_DESC target_array_desc = target_desc;
    target_array_desc.DepthOrArraySize = 2;
    HRESULT target_array_hr =
        SUCCEEDED(target_hr) ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &target_array_desc,
                                                               D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
                                                               IID_PPV_ARGS(&target_array))
                             : E_FAIL;
    ...
    HRESULT target_2_hr =
        SUCCEEDED(target_array_hr)
            ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &target_2_desc,
                                              D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&target_2))
            : E_FAIL;
    ...
    HRESULT target_8_hr =
        SUCCEEDED(target_2_hr)
            ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &target_8_desc,
                                              D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&target_8))
            : E_FAIL;
    D3D12_RESOURCE_DESC target_r8_desc = target_desc;
    target_r8_desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    HRESULT target_r8_hr =
        SUCCEEDED(target_8_hr)
            ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &target_r8_desc,
                                              D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&target_r8))
            : E_FAIL;
    D3D12_RESOURCE_DESC resolve_desc = target_desc;
    resolve_desc.SampleDesc.Count = 1;
    resolve_desc.Flags = D3D12_RESOURCE_FLAG_NONE;
    HRESULT resolve_target_hr =
        SUCCEEDED(target_array_hr)
            ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &resolve_desc,
                                              D3D12_RESOURCE_STATE_RESOLVE_DEST, nullptr, IID_PPV_ARGS(&resolve_target))
            : E_FAIL;
    D3D12_RESOURCE_DESC resolve_array_desc = resolve_desc;
    resolve_array_desc.DepthOrArraySize = 2;
    HRESULT resolve_array_target_hr =
        SUCCEEDED(resolve_target_hr)
            ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &resolve_array_desc,
    ...
    HRESULT resolve_2_target_hr = SUCCEEDED(resolve_array_target_hr)
                                      ? device->CreateCommittedResource(
                                            &default_heap, D3D12_HEAP_FLAG_NONE, &resolve_2_desc,
                                            D3D12_RESOURCE_STATE_RESOLVE_DEST, nullptr, IID_PPV_ARGS(&resolve_2_target))
                                      : E_FAIL;
    HRESULT resolve_8_target_hr = SUCCEEDED(resolve_2_target_hr)
                                      ? device->CreateCommittedResource(
                                            &default_heap, D3D12_HEAP_FLAG_NONE, &resolve_8_desc,
                                            D3D12_RESOURCE_STATE_RESOLVE_DEST, nullptr, IID_PPV_ARGS(&resolve_8_target))
                                      : E_FAIL;
    D3D12_RESOURCE_DESC resolve_r8_desc = resolve_desc;
    resolve_r8_desc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    HRESULT resolve_r8_target_hr =
        SUCCEEDED(resolve_8_target_hr)
            ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &resolve_r8_desc,
                                              D3D12_RESOURCE_STATE_RESOLVE_DEST, nullptr,
    ...
    HRESULT graphics_target_hr = SUCCEEDED(readback_hr)
                                     ? device->CreateCommittedResource(
                                           &default_heap, D3D12_HEAP_FLAG_NONE, &graphics_target_desc,
                                           D3D12_RESOURCE_STATE_RENDER_TARGET, nullptr, IID_PPV_ARGS(&graphics_target))
                                     : E_FAIL;
    ...
    HRESULT depth_target_hr = SUCCEEDED(graphics_target_hr)
                                  ? device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &depth_desc,
    ...
    HRESULT graphics_readback_hr =
        SUCCEEDED(vertex_buffer_hr)
            ? device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &graphics_readback_desc,
                                              D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&graphics_readback))
            : E_FAIL;
    HRESULT resolve_readback_hr =
        SUCCEEDED(graphics_readback_hr)
            ? device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &graphics_readback_desc,
                                              D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&resolve_readback))
            : E_FAIL;
    HRESULT resolve_array_readback_hr =
        SUCCEEDED(resolve_readback_hr)
            ? device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &graphics_readback_desc,
                                              D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                              IID_PPV_ARGS(&resolve_array_readback))
            : E_FAIL;
    HRESULT resolve_2_readback_hr =
        SUCCEEDED(resolve_array_readback_hr)
            ? device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &graphics_readback_desc,
                                              D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                              IID_PPV_ARGS(&resolve_2_readback))
            : E_FAIL;
    HRESULT resolve_8_readback_hr =
        SUCCEEDED(resolve_2_readback_hr)
            ? device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &graphics_readback_desc,
                                              D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                              IID_PPV_ARGS(&resolve_8_readback))
            : E_FAIL;
    HRESULT resolve_r8_readback_hr =
    ...
    if (SUCCEEDED(rtv_heap_hr) && rtv_heap) {
        ...
        device->CreateRenderTargetView(graphics_target, &rtv_desc, rtv_heap->GetCPUDescriptorHandleForHeapStart());
    }
    D3D12_DESCRIPTOR_HEAP_DESC dsv_heap_desc = {};
    dsv_heap_desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_DSV;
    dsv_heap_desc.NumDescriptors = 1;
    HRESULT dsv_heap_hr =
        SUCCEEDED(rtv_heap_hr) ? device->CreateDescriptorHeap(&dsv_heap_desc, IID_PPV_ARGS(&dsv_heap)) : E_FAIL;
    if (SUCCEEDED(dsv_heap_hr) && dsv_heap) {
        D3D12_DEPTH_STENCIL_VIEW_DESC dsv_desc = {};
        dsv_desc.Format = DXGI_FORMAT_D32_FLOAT;
        dsv_desc.ViewDimension = D3D12_DSV_DIMENSION_TEXTURE2D;
        dsv_desc.Flags = D3D12_DSV_FLAG_NONE;
        device->CreateDepthStencilView(depth_target, &dsv_desc, dsv_heap->GetCPUDescriptorHandleForHeapStart());
    }

    HRESULT execute_hr = E_FAIL;
    if (SUCCEEDED(readback_hr) && store_pso && load_pso && store_2_pso && load_2_pso && store_8_pso && load_8_pso &&
        graphics_pso && SUCCEEDED(graphics_target_hr) && SUCCEEDED(depth_target_hr) &&
        SUCCEEDED(graphics_readback_hr) && SUCCEEDED(resolve_readback_hr) && SUCCEEDED(resolve_array_readback_hr) &&
        SUCCEEDED(resolve_2_readback_hr) && SUCCEEDED(resolve_8_readback_hr) && SUCCEEDED(rtv_heap_hr) &&
        SUCCEEDED(dsv_heap_hr) && SUCCEEDED(resolve_target_hr) && SUCCEEDED(resolve_array_target_hr) &&
        SUCCEEDED(resolve_2_target_hr) && SUCCEEDED(resolve_8_target_hr) && vertex_buffer) {
        ...
        device->CreateUnorderedAccessView(target_array, nullptr, &target_array_uav, cpu);
        cpu.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        ...
        list->Dispatch(1, 1, 1);
        ID3D12Resource* sample_targets[] = {target, target_array, target_2, target_8, target_r8};
        ...
        D3D12_CPU_DESCRIPTOR_HANDLE graphics_rtv = rtv_heap->GetCPUDescriptorHandleForHeapStart();
        ...
        graphics_target_barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        graphics_target_barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_RENDER_TARGET;
        graphics_target_barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_SOURCE;
        ...
        graphics_dst.PlacedFootprint.Footprint.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
        ...
        list->ResolveSubresource(resolve_target, 0, target, 0, DXGI_FORMAT_R32G32B32A32_FLOAT);
        ...
        resolve_barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        resolve_barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_RESOLVE_DEST;
        resolve_barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_SOURCE;
        ...
        resolve_dst.PlacedFootprint.Footprint.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
        ...
        list->ResolveSubresource(resolve_array_target, 1, target_array, 1, DXGI_FORMAT_R32G32B32A32_FLOAT);
        ...
        resolve_array_barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        resolve_array_barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_RESOLVE_DEST;
        resolve_array_barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_SOURCE;
        ...
        resolve_array_dst.PlacedFootprint.Footprint.Format = DXGI_FORMAT_R32G32B32A32_FLOAT;
        ...
        list->CopyTextureRegion(&resolve_array_dst, 0, 0, 0, &resolve_array_src, nullptr);

        auto resolve_and_copy = [&](ID3D12Resource* source, ID3D12Resource* destination,
                                    ID3D12Resource* destination_readback, DXGI_FORMAT format) {
            ...
            list->CopyTextureRegion(&destination_location, 0, 0, 0, &source_location, nullptr);
        };
        resolve_and_copy(target_2, resolve_2_target, resolve_2_readback, DXGI_FORMAT_R16G16B16A16_FLOAT);
        resolve_and_copy(target_8, resolve_8_target, resolve_8_readback, DXGI_FORMAT_R16G16B16A16_FLOAT);
        resolve_and_copy(target_r8, resolve_r8_target, resolve_r8_readback, DXGI_FORMAT_R8G8B8A8_UNORM);
    ...
    if (SUCCEEDED(execute_hr) && graphics_readback) {
        ...
        graphics_map_hr = graphics_readback->Map(0, &read_range, &mapped);
    ...
    const bool values_ok = values[0] == 318 && values[1] == 308 && values[2] == 320 && values[3] == 321 &&
                           ...
                           values[16] == 606 && values[17] == 607 && values[18] == 192 && values[19] == 65 &&
    ...
    if (SUCCEEDED(execute_hr) && resolve_array_readback) {
        void* mapped = nullptr;
        D3D12_RANGE read_range = {0, 256};
        resolve_array_map_hr = resolve_array_readback->Map(0, &read_range, &mapped);
    ...
    const bool graphics_color_ok = graphics_color == 0xff4080ffu;
    const bool resolve_value_ok = resolve_value == 316.75f;
    ...
    const bool pass =
        source_ok && dxc_store == 0 && dxc_load == 0 && dxc_store_2 == 0 && dxc_load_2 == 0 && dxc_store_8 == 0 &&
        dxc_load_8 == 0 && dxc_store_r8 == 0 && dxc_load_r8 == 0 && dxc_graphics_vs == 0 && dxc_graphics_ps == 0 &&
        !store_dxil.empty() && !load_dxil.empty() && !store_2_dxil.empty() && !load_2_dxil.empty() &&
        !store_8_dxil.empty() && !load_8_dxil.empty() && !store_r8_dxil.empty() && !load_r8_dxil.empty() &&
        !graphics_vs_dxil.empty() && !graphics_ps_dxil.empty() && SUCCEEDED(create_hr) &&
        SUCCEEDED(root_serialize_hr) && SUCCEEDED(root_create_hr) && SUCCEEDED(store_pso_hr) &&
        ...
        SUCCEEDED(resolve_8_target_hr) && SUCCEEDED(resolve_r8_target_hr) && SUCCEEDED(graphics_target_hr) &&
        SUCCEEDED(depth_target_hr) && SUCCEEDED(vertex_buffer_hr) && SUCCEEDED(graphics_readback_hr) &&
        SUCCEEDED(resolve_readback_hr) && SUCCEEDED(resolve_array_readback_hr) && SUCCEEDED(resolve_2_readback_hr) &&
        SUCCEEDED(resolve_8_readback_hr) && SUCCEEDED(resolve_r8_readback_hr) && SUCCEEDED(rtv_heap_hr) &&
        SUCCEEDED(dsv_heap_hr) && SUCCEEDED(execute_hr) && SUCCEEDED(map_hr) && values_ok &&
    ...
    const bool options14_behavior_complete = pass && resolve_value_ok && resolve_array_value_ok && resolve_2_value_ok &&
                                             resolve_8_value_ok && resolve_r8_value_ok && graphics_color_ok;
