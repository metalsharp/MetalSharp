...
template <D3D12_PIPELINE_STATE_SUBOBJECT_TYPE Type, typename T> struct alignas(void*) PipelineStreamSubobject {
...
static ProbeResult probe_inner_coverage() {
    std::string vs_path = getenv_string("D3D12_METAL_SDK_INNER_COVERAGE_VS");
    std::string ps_path = getenv_string("D3D12_METAL_SDK_INNER_COVERAGE_PS");
    if (vs_path.empty())
        vs_path = "probe_inner_coverage_vs.cso";
    if (ps_path.empty())
        ps_path = "probe_inner_coverage_ps.cso";

    std::vector<uint8_t> vs_blob;
    std::vector<uint8_t> ps_blob;
    if (!read_binary_file(vs_path, vs_blob) || !read_binary_file(ps_path, ps_blob)) {
        return {false, HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND),
                "inner-coverage shader blobs are missing",
                "\"vs_path\":\"" + json_escape(vs_path) +
                    "\",\"ps_path\":\"" + json_escape(ps_path) + "\""};
    }
    ...
    if (SUCCEEDED(hr)) {
        ...
        view.SizeInBytes = sizeof(vertex_data);
    ...
    uint64_t inner_pixels = 0;
    uint64_t outer_pixels = 0;
    uint64_t unexpected_pixels = 0;
    uint32_t center_pixel = 0;
    if (SUCCEEDED(hr)) {
        uint8_t* mapped = nullptr;
    ...
    safe_release(queue);
    safe_release(device);
    constexpr uint64_t expected_inner_pixels = 1200;
    constexpr uint64_t expected_outer_pixels = 204;
    const bool verified = SUCCEEDED(hr) && inner_pixels == expected_inner_pixels &&
                          outer_pixels == expected_outer_pixels &&
                          unexpected_pixels == 0 && center_pixel == 0xffffffffu;
    return {verified, verified ? S_OK : hr,
            verified ? "SV_InnerCoverage matched exact conservative raster readback"
                     : (detail.empty() ? "inner-coverage exact raster readback failed" : detail),
            "\"inner_pixels\":" + std::to_string(inner_pixels) +
                ",\"expected_inner_pixels\":" +
                std::to_string(expected_inner_pixels) +
                ",\"outer_pixels\":" + std::to_string(outer_pixels) +
                ",\"expected_outer_pixels\":" +
                std::to_string(expected_outer_pixels) +
                ",\"unexpected_pixels\":" +
                std::to_string(unexpected_pixels) +
                ",\"center_pixel\":" + std::to_string(center_pixel) +
...
static ProbeResult probe_mesh_shader_pso() {
    ...
    if (!read_binary_file("probe_mesh_shader_as.cso", amplification_shader) ||
        ...
        return {false, HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND), "mesh/pixel shader probe blobs are missing",
                "\"pso_attempted\":false"};
    }
    ...
    const HRESULT options7_hr = device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS7, &options7, sizeof(options7));
    D3D12_FEATURE_DATA_D3D12_OPTIONS2 options2 = {};
    const HRESULT options2_hr = device->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS2, &options2, sizeof(options2));
    ...
    struct alignas(void*) MeshPipelineStream {
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_ROOT_SIGNATURE, ID3D12RootSignature*> root;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_AS, D3D12_SHADER_BYTECODE> amplification;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_MS, D3D12_SHADER_BYTECODE> mesh;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_PS, D3D12_SHADER_BYTECODE> pixel;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_BLEND, D3D12_BLEND_DESC> blend;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_SAMPLE_MASK, UINT> sample_mask;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_RASTERIZER, D3D12_RASTERIZER_DESC> rasterizer;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL1, D3D12_DEPTH_STENCIL_DESC1>
            depth_stencil;
        PipelineStreamSubobject<D3D12_PIPELINE_STATE_SUBOBJECT_TYPE_DEPTH_STENCIL_FORMAT, DXGI_FORMAT>
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            depth_stream.depth_stencil.value.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;
            depth_stream.depth_stencil.value.DepthFunc = D3D12_COMPARISON_FUNC_LESS;
            depth_stream.depth_stencil_format.value = DXGI_FORMAT_D32_FLOAT;
            D3D12_PIPELINE_STATE_STREAM_DESC depth_desc = {sizeof(depth_stream), &depth_stream};
            hr = device2->CreatePipelineState(&depth_desc, IID_PPV_ARGS(&depth_pso));
            if (SUCCEEDED(hr)) {
                depth_stream.depth_stencil.value.DepthBoundsTestEnable = FALSE;
                hr = device2->CreatePipelineState(&depth_desc, IID_PPV_ARGS(&depth_disabled_pso));
            }
        }
        if (SUCCEEDED(hr)) {
            ...
            D3D12_PIPELINE_STATE_STREAM_DESC blend_desc = {sizeof(blend_stream), &blend_stream};
            hr = device2->CreatePipelineState(&blend_desc, IID_PPV_ARGS(&blend_pso));
        }
        if (SUCCEEDED(hr)) {
            MeshPipelineStream wireframe_stream = stream;
            wireframe_stream.rasterizer.value.FillMode = D3D12_FILL_MODE_WIREFRAME;
            D3D12_PIPELINE_STATE_STREAM_DESC wireframe_desc = {sizeof(wireframe_stream), &wireframe_stream};
            hr = device2->CreatePipelineState(&wireframe_desc, IID_PPV_ARGS(&wireframe_pso));
        }
    }
    if (SUCCEEDED(hr))
        hr = create_queue(device, D3D12_COMMAND_LIST_TYPE_DIRECT, &queue);
    ...
    if (SUCCEEDED(hr)) {
        ...
        query_desc.Count = 1;
        hr = device->CreateQueryHeap(&query_desc, IID_PPV_ARGS(&pipeline_statistics1_heap));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC stats_desc = buffer_desc(sizeof(D3D12_QUERY_DATA_PIPELINE_STATISTICS1));
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &stats_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        device->GetCopyableFootprints(&target_desc, 0, 2, 0, footprints, rows, row_bytes, &readback_bytes);
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC readback_desc = buffer_desc(readback_bytes);
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &readback_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&readback));
    ...
    if (SUCCEEDED(hr)) {
        ...
        clear.Color[0] = 1.0f;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &target_desc,
                                             D3D12_RESOURCE_STATE_RENDER_TARGET, &clear, IID_PPV_ARGS(&depth_target));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC readback_desc = buffer_desc(readback_bytes);
    ...
    if (SUCCEEDED(hr)) {
        ...
        clear.Color[3] = 0.5f;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &target_desc,
                                             D3D12_RESOURCE_STATE_RENDER_TARGET, &clear, IID_PPV_ARGS(&blend_target));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC readback_desc = buffer_desc(readback_bytes);
    ...
    if (SUCCEEDED(hr)) {
        D3D12_RESOURCE_DESC depth_desc =
            texture_desc(64, 64, DXGI_FORMAT_D32_FLOAT, D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL);
        ...
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &depth_desc,
                                             D3D12_RESOURCE_STATE_DEPTH_WRITE, &clear, IID_PPV_ARGS(&depth_texture));
    }
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommandSignature(&signature_desc, nullptr, IID_PPV_ARGS(&mesh_signature));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES upload_heap = heap_props(D3D12_HEAP_TYPE_UPLOAD);
        D3D12_RESOURCE_DESC args_desc = buffer_desc(sizeof(D3D12_DISPATCH_MESH_ARGUMENTS));
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &args_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&indirect_args));
        void* mapped = nullptr;
        if (SUCCEEDED(hr))
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr =
            device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &constants_desc,
                                            D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&stage_constants));
        uint8_t* mapped = nullptr;
        if (SUCCEEDED(hr))
        ...
        if (SUCCEEDED(hr)) {
            ...
            std::memcpy(mapped + 256, &amplification_enabled, sizeof(amplification_enabled));
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &srv_buffer_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&stage_srvs));
        uint8_t* mapped = nullptr;
        if (SUCCEEDED(hr))
        ...
        if (SUCCEEDED(hr)) {
            ...
            std::memcpy(mapped + 256, &amplification_control, sizeof(amplification_control));
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr =
            device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &output_desc,
                                            D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&mesh_output));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC output_desc = buffer_desc(256);
    ...
    D3D12_RESOURCE_DESC mesh_texture_desc = texture_desc(1, 1, DXGI_FORMAT_R32_FLOAT, D3D12_RESOURCE_FLAG_NONE);
    if (SUCCEEDED(hr)) {
        device->GetCopyableFootprints(&mesh_texture_desc, 0, 1, 0, &mesh_texture_footprint, &mesh_texture_rows,
                                      &mesh_texture_row_bytes, &mesh_texture_upload_bytes);
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &mesh_texture_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&mesh_texture));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES upload_heap = heap_props(D3D12_HEAP_TYPE_UPLOAD);
        ...
        uint8_t* mapped = nullptr;
        if (SUCCEEDED(hr))
            hr = mesh_texture_upload->Map(0, nullptr, reinterpret_cast<void**>(&mapped));
        if (SUCCEEDED(hr)) {
            const float value = 0.5f;
            std::memcpy(mapped + mesh_texture_footprint.Offset, &value, sizeof(value));
    ...
    if (SUCCEEDED(hr)) {
        ...
        depth_rtv.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        device->CreateRenderTargetView(depth_target, &rtv_desc, depth_rtv);
        D3D12_CPU_DESCRIPTOR_HANDLE blend_rtv = depth_rtv;
        blend_rtv.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        device->CreateRenderTargetView(blend_target, &rtv_desc, blend_rtv);
        D3D12_CPU_DESCRIPTOR_HANDLE wireframe_rtv = blend_rtv;
        wireframe_rtv.ptr += device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        device->CreateRenderTargetView(wireframe_target, &rtv_desc, wireframe_rtv);
        ...
        list6->SetGraphicsRootConstantBufferView(0, stage_constants->GetGPUVirtualAddress());
        list6->SetGraphicsRootConstantBufferView(1, stage_constants->GetGPUVirtualAddress() + 256);
        ...
        list6->SetGraphicsRootDescriptorTable(6, sampler_heap->GetGPUDescriptorHandleForHeapStart());
        ...
        D3D12_RESOURCE_BARRIER texture_barrier = transition_barrier(mesh_texture, D3D12_RESOURCE_STATE_COPY_DEST,
        ...
        list6->BeginQuery(pipeline_statistics1_heap, D3D12_QUERY_TYPE_PIPELINE_STATISTICS1, 0);
        list6->DispatchMesh(2, 1, 1);
        list6->EndQuery(pipeline_statistics1_heap, D3D12_QUERY_TYPE_PIPELINE_STATISTICS1, 0);
        list6->ResolveQueryData(pipeline_statistics1_heap, D3D12_QUERY_TYPE_PIPELINE_STATISTICS1, 0, 1,
                                pipeline_statistics1_readback, 0);
        ...
        D3D12_RESOURCE_BARRIER output_uav_barrier =
            transition_barrier(mesh_output, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        ...
        list6->ClearDepthStencilView(dsv, D3D12_CLEAR_FLAG_DEPTH, 0.5f, 0, 0, nullptr);
        D3D12_RECT full_scissor = {0, 0, 64, 64};
        list6->RSSetScissorRects(1, &full_scissor);
        list6->OMSetDepthBounds(0.4f, 0.6f);
        list6->DispatchMesh(2, 1, 1);
        D3D12_RESOURCE_BARRIER depth_target_barrier =
            transition_barrier(depth_target, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE);
        ...
        D3D12_RESOURCE_BARRIER depth_target_reset_barrier =
            transition_barrier(depth_target, D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_RENDER_TARGET);
        ...
        list6->DispatchMesh(2, 1, 1);
        D3D12_RESOURCE_BARRIER depth_reject_target_barrier =
            transition_barrier(depth_target, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE);
        ...
        list6->RSSetScissorRects(1, &full_scissor);
        list6->DispatchMesh(2, 1, 1);
        D3D12_RESOURCE_BARRIER blend_target_barrier =
            transition_barrier(blend_target, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE);
        ...
        list6->RSSetScissorRects(1, &full_scissor);
        list6->DispatchMesh(2, 1, 1);
        D3D12_RESOURCE_BARRIER wireframe_target_barrier =
            transition_barrier(wireframe_target, D3D12_RESOURCE_STATE_RENDER_TARGET, D3D12_RESOURCE_STATE_COPY_SOURCE);
    ...
    if (SUCCEEDED(hr)) {
        void* mapped = nullptr;
        D3D12_RANGE stats_range = {0, sizeof(D3D12_QUERY_DATA_PIPELINE_STATISTICS1)};
        hr = pipeline_statistics1_readback->Map(0, &stats_range, &mapped);
        if (SUCCEEDED(hr) && mapped) {
            std::memcpy(&pipeline_statistics1, mapped, sizeof(pipeline_statistics1));
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            for (UINT slice = 0; slice < 2; slice++) {
                for (UINT y = 0; y < 64; y++) {
                    const uint32_t* row = reinterpret_cast<const uint32_t*>(mapped + footprints[slice].Offset +
                                                                            footprints[slice].Footprint.RowPitch * y);
                    for (UINT x = 0; x < 64; x++) {
                        const bool rendered = row[x] == 0x80bf8040u;
                        clear_pixels += row[x] == 0xff0000ffu;
                        const bool unexpected = row[x] != 0x80bf8040u && row[x] != 0xff0000ffu;
    ...
    if (SUCCEEDED(hr)) {
        uint8_t* mapped = nullptr;
        D3D12_RANGE read_range = {0, static_cast<SIZE_T>(readback_bytes)};
        hr = depth_target_readback->Map(0, &read_range, reinterpret_cast<void**>(&mapped));
        if (SUCCEEDED(hr)) {
            for (UINT slice = 0; slice < 2; slice++) {
                for (UINT y = 0; y < 64; y++) {
                    const uint32_t* row = reinterpret_cast<const uint32_t*>(mapped + footprints[slice].Offset +
                    ...
                    for (UINT x = 0; x < 64; x++) {
                        ...
                        depth_unexpected_pixels += row[x] != 0x80bf8040u && row[x] != 0xff0000ffu;
                    }
                }
            }
    ...
    if (SUCCEEDED(hr)) {
        uint8_t* mapped = nullptr;
        D3D12_RANGE read_range = {0, static_cast<SIZE_T>(readback_bytes)};
        hr = depth_reject_readback->Map(0, &read_range, reinterpret_cast<void**>(&mapped));
        if (SUCCEEDED(hr)) {
            for (UINT slice = 0; slice < 2; slice++) {
                for (UINT y = 0; y < 64; y++) {
                    const uint32_t* row = reinterpret_cast<const uint32_t*>(mapped + footprints[slice].Offset +
                    ...
                    for (UINT x = 0; x < 64; x++) {
                        ...
                        depth_reject_unexpected_pixels += row[x] != 0xff0000ffu;
                    }
                }
            }
    ...
    if (SUCCEEDED(hr)) {
        uint8_t* mapped = nullptr;
        D3D12_RANGE read_range = {0, static_cast<SIZE_T>(readback_bytes * 2)};
        hr = depth_disabled_readback->Map(0, &read_range, reinterpret_cast<void**>(&mapped));
        if (SUCCEEDED(hr)) {
            for (UINT slice = 0; slice < 2; slice++) {
                for (UINT y = 0; y < 64; y++) {
                    const uint32_t* inverted_row = reinterpret_cast<const uint32_t*>(
                        mapped + footprints[slice].Offset + footprints[slice].Footprint.RowPitch * y);
                    const uint32_t* row = reinterpret_cast<const uint32_t*>(
                        mapped + readback_bytes + footprints[slice].Offset + footprints[slice].Footprint.RowPitch * y);
                    for (UINT x = 0; x < 64; x++) {
                        depth_inverted_clear_pixels += inverted_row[x] == 0xff0000ffu;
                        depth_inverted_unexpected_pixels += inverted_row[x] != 0xff0000ffu;
                        depth_disabled_layer_pixels[slice] += row[x] == 0x80bf8040u;
                        depth_disabled_clear_pixels += row[x] == 0xff0000ffu;
                        depth_disabled_unexpected_pixels += row[x] != 0x80bf8040u && row[x] != 0xff0000ffu;
                    }
                }
            }
    ...
    if (SUCCEEDED(hr)) {
        uint8_t* mapped = nullptr;
        D3D12_RANGE read_range = {0, static_cast<SIZE_T>(readback_bytes)};
        hr = blend_target_readback->Map(0, &read_range, reinterpret_cast<void**>(&mapped));
        if (SUCCEEDED(hr)) {
            for (UINT slice = 0; slice < 2; slice++) {
                for (UINT y = 0; y < 64; y++) {
                    const uint32_t* row = reinterpret_cast<const uint32_t*>(mapped + footprints[slice].Offset +
                    ...
                    for (UINT x = 0; x < 64; x++) {
                        ...
                        blend_unexpected_pixels += row[x] != 0xffffff80u && row[x] != 0x80408040u;
                    }
                }
            }
    ...
    if (SUCCEEDED(hr)) {
        uint8_t* mapped = nullptr;
        D3D12_RANGE read_range = {0, static_cast<SIZE_T>(readback_bytes)};
        hr = wireframe_target_readback->Map(0, &read_range, reinterpret_cast<void**>(&mapped));
        if (SUCCEEDED(hr)) {
            for (UINT slice = 0; slice < 2; slice++) {
                for (UINT y = 0; y < 64; y++) {
                    const uint32_t* row = reinterpret_cast<const uint32_t*>(mapped + footprints[slice].Offset +
                                                                            footprints[slice].Footprint.RowPitch * y);
                    for (UINT x = 0; x < 64; x++) {
                        wireframe_layer_pixels[slice] += row[x] == 0x80bf8040u;
                        wireframe_clear_pixels += row[x] == 0xff0000ffu;
                        wireframe_unexpected_pixels += row[x] != 0x80bf8040u && row[x] != 0xff0000ffu;
                    }
                }
            }
    ...
    const bool verified =
        ...
        layer_direct_pixels[1] <= 400 && layer_indirect_pixels[0] >= 100 && layer_indirect_pixels[0] <= 400 &&
        layer_indirect_pixels[1] >= 100 && layer_indirect_pixels[1] <= 400 &&
        clear_pixels + nonzero_pixels == 64u * 64u * 2u && unexpected_pixels == 0 && depth_layer_pixels[0] >= 300 &&
        depth_layer_pixels[0] <= 400 && depth_layer_pixels[1] == 0 &&
        depth_clear_pixels + depth_layer_pixels[0] == 64u * 64u * 2u && depth_unexpected_pixels == 0 &&
        depth_reject_clear_pixels == 64u * 64u * 2u && depth_reject_unexpected_pixels == 0 &&
        depth_inverted_clear_pixels == 64u * 64u * 2u && depth_inverted_unexpected_pixels == 0 &&
        depth_disabled_layer_pixels[0] >= 300 && depth_disabled_layer_pixels[0] <= 400 &&
        depth_disabled_layer_pixels[1] == 0 &&
        depth_disabled_clear_pixels + depth_disabled_layer_pixels[0] == 64u * 64u * 2u &&
        depth_disabled_unexpected_pixels == 0 && blend_layer_pixels[0] >= 300 && blend_layer_pixels[0] <= 400 &&
        blend_layer_pixels[1] >= 300 && blend_layer_pixels[1] <= 400 &&
        blend_clear_pixels + blend_layer_pixels[0] + blend_layer_pixels[1] == 64u * 64u * 2u &&
        blend_unexpected_pixels == 0 && wireframe_layer_pixels[0] >= 10 &&
        wireframe_layer_pixels[0] < layer_pixels[0] && wireframe_layer_pixels[1] >= 10 &&
    ...
    return {
        verified, verified ? S_OK : hr,
        verified ? "native D3D12 AS/MS direct and indirect DispatchMesh rendered with PIPELINE_STATISTICS1; tier "
                   "remains conservative"
                 : (detail.empty() ? "native mesh shader dispatch/readback failed" : detail),
        "\"pso_attempted\":true,\"repeated_pso_created\":true" + std::string(",\"depth_pso_created\":true") +
            std::string(",\"depth_disabled_pso_created\":true") + std::string(",\"depth_bounds_supported\":") +
            (options2.DepthBoundsTestSupported ? "true" : "false") + std::string(",\"blend_pso_created\":true") +
            std::string(",\"wireframe_pso_created\":true") + std::string(",\"stage_cbvs_bound\":true") +
            std::string(",\"stage_srvs_bound\":true") + std::string(",\"mesh_uav_bound\":true") +
            std::string(",\"mesh_texture_sampler_bound\":true") + std::string(",\"amplification_shader_bytes\":") +
            ...
            ",\"layer1_direct_pixels\":" + std::to_string(layer_direct_pixels[1]) +
            ",\"layer0_indirect_pixels\":" + std::to_string(layer_indirect_pixels[0]) + ",\"layer1_indirect_pixels\":" +
            std::to_string(layer_indirect_pixels[1]) + ",\"array_clear_pixels\":" + std::to_string(clear_pixels) +
            ",\"unexpected_pixels\":" + std::to_string(unexpected_pixels) + ",\"unexpected_pixel_sample\":" +
            std::to_string(unexpected_pixel_sample) + ",\"mesh_color_rgba8\":\"0x80bf8040\"" +
            ",\"clear_color_rgba8\":\"0xff0000ff\"" + ",\"depth_clear\":0.5" + ",\"depth_layer0_value\":0.25" +
            ",\"depth_layer1_value\":0.75" + ",\"depth_layer0_pixels\":" + std::to_string(depth_layer_pixels[0]) +
            ",\"depth_layer1_pixels\":" + std::to_string(depth_layer_pixels[1]) +
            ",\"depth_clear_pixels\":" + std::to_string(depth_clear_pixels) +
            ",\"depth_unexpected_pixels\":" + std::to_string(depth_unexpected_pixels) +
            ",\"depth_bounds_accept_min\":0.4" + ",\"depth_bounds_accept_max\":0.6" +
            ",\"depth_bounds_reject_min\":0.6" + ",\"depth_bounds_reject_max\":0.9" +
            ",\"depth_reject_clear_pixels\":" + std::to_string(depth_reject_clear_pixels) +
            ",\"depth_reject_unexpected_pixels\":" + std::to_string(depth_reject_unexpected_pixels) +
            ",\"depth_inverted_bounds_min\":0.9" + ",\"depth_inverted_bounds_max\":0.1" +
            ",\"depth_inverted_clear_pixels\":" + std::to_string(depth_inverted_clear_pixels) +
            ",\"depth_inverted_unexpected_pixels\":" + std::to_string(depth_inverted_unexpected_pixels) +
            ",\"depth_disabled_bounds_min\":0.9" + ",\"depth_disabled_bounds_max\":0.1" +
            ",\"depth_disabled_layer0_pixels\":" + std::to_string(depth_disabled_layer_pixels[0]) +
            ",\"depth_disabled_layer1_pixels\":" + std::to_string(depth_disabled_layer_pixels[1]) +
            ",\"depth_disabled_clear_pixels\":" + std::to_string(depth_disabled_clear_pixels) +
            ",\"depth_disabled_unexpected_pixels\":" + std::to_string(depth_disabled_unexpected_pixels) +
            ",\"blend_source_rgba8\":\"0x80bf8040\"" + ",\"blend_clear_rgba8\":\"0x80408040\"" +
            ",\"blend_expected_rgba8\":\"0xffffff80\"" +
            ",\"blend_layer0_pixels\":" + std::to_string(blend_layer_pixels[0]) + ",\"blend_layer1_pixels\":" +
            std::to_string(blend_layer_pixels[1]) + ",\"blend_clear_pixels\":" + std::to_string(blend_clear_pixels) +
            ",\"blend_unexpected_pixels\":" + std::to_string(blend_unexpected_pixels) +
            ",\"wireframe_layer0_pixels\":" + std::to_string(wireframe_layer_pixels[0]) +
            ",\"wireframe_layer1_pixels\":" + std::to_string(wireframe_layer_pixels[1]) +
            ...
            ",\"pipeline_statistics1_ms_primitives\":" + std::to_string(pipeline_statistics1.MSPrimitives) +
            ",\"tier1_matrix_complete\":true" + ",\"d3d12_loaded_path\":\"" + json_escape(g_d3d12_loaded_path) + "\""};
}
...
static ProbeResult probe_dxr_acceleration_structures() {
    ...
    if (!read_binary_file("probe_dxr_inline.cso", ray_query_shader) ||
        ...
        return {false, HRESULT_FROM_WIN32(ERROR_FILE_NOT_FOUND), "inline ray-query or raygen shader blob is missing",
                ""};
    }
    ID3D12Device* device = nullptr;
    HRESULT hr = create_device(&device);
    ...
    ID3D12Resource* mixed_geometry_acceleration_structure = nullptr;
    ID3D12Resource* mixed_geometry_scratch = nullptr;
    ID3D12Resource* mixed_geometry_postbuild = nullptr;
    ...
    if (SUCCEEDED(hr))
        hr = serialize_root_signature(compute_root_desc, &compute_root_blob, compute_detail);
    if (SUCCEEDED(hr))
        hr = device->CreateRootSignature(0, compute_root_blob->GetBufferPointer(), compute_root_blob->GetBufferSize(),
                                         IID_PPV_ARGS(&compute_root));
    if (SUCCEEDED(hr)) {
        D3D12_COMPUTE_PIPELINE_STATE_DESC compute_desc = {};
        compute_desc.pRootSignature = compute_root;
        compute_desc.CS = {ray_query_shader.data(), ray_query_shader.size()};
        hr = device->CreateComputePipelineState(&compute_desc, IID_PPV_ARGS(&compute_pso));
    }
    safe_release(compute_root_blob);

    D3D12_ROOT_PARAMETER local_root_params[5] = {};
    local_root_params[0].ParameterType = D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;
    ...
    local_root_params[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    ...
    local_root_params[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    ...
    local_root_params[3].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    ...
    local_root_params[4].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    ...
    if (SUCCEEDED(hr))
        hr = serialize_root_signature(local_root_desc, &local_root_blob, compute_detail);
    if (SUCCEEDED(hr))
        hr = device->CreateRootSignature(0, local_root_blob->GetBufferPointer(), local_root_blob->GetBufferSize(),
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &local_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &local_desc,
    ...
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC local_desc = buffer_desc(256);
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &local_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &local_texture_desc,
    ...
    if (SUCCEEDED(hr)) {
        ...
        rtv_heap_desc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;
        rtv_heap_desc.NumDescriptors = 1;
        hr = device->CreateDescriptorHeap(&rtv_heap_desc, IID_PPV_ARGS(&local_texture_rtv_heap));
    }
    if (SUCCEEDED(hr)) {
        device->CreateRenderTargetView(closest_hit_local_texture, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        sampler_heap_desc.NumDescriptors = 1;
        sampler_heap_desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
        hr = device->CreateDescriptorHeap(&sampler_heap_desc, IID_PPV_ARGS(&local_sampler_heap));
    }
    if (SUCCEEDED(hr)) {
        ...
        device->CreateSampler(&sampler_desc, local_sampler_heap->GetCPUDescriptorHandleForHeapStart());
    }
    ...
    raygen_library_desc.DXILLibrary = {raygen_library.data(), raygen_library.size()};
    ...
    LPCWSTR local_root_exports[] = {L"hit_group", L"miss_shader", L"miss_alias", L"callable_shader", L"callable_alias"};
    D3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION local_root_association = {};
    D3D12_STATE_SUBOBJECT state_subobjects[8] = {};
    state_subobjects[0] = {D3D12_STATE_SUBOBJECT_TYPE_DXIL_LIBRARY, &raygen_library_desc};
    state_subobjects[1] = {D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE, &global_root};
    state_subobjects[2] = {D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_SHADER_CONFIG, &shader_config};
    state_subobjects[3] = {D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_PIPELINE_CONFIG, &pipeline_config};
    state_subobjects[4] = {D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP, &hit_group};
    state_subobjects[5] = {D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP, &procedural_hit_group};
    state_subobjects[6] = {D3D12_STATE_SUBOBJECT_TYPE_LOCAL_ROOT_SIGNATURE, &local_root};
    ...
    state_subobjects[7] = {D3D12_STATE_SUBOBJECT_TYPE_SUBOBJECT_TO_EXPORTS_ASSOCIATION, &local_root_association};
    ...
    if (SUCCEEDED(hr))
        hr = device5->CreateStateObject(&state_desc, IID_PPV_ARGS(&raytracing_collection));
    ...
    D3D12_STATE_SUBOBJECT filtered_subobject_a = {D3D12_STATE_SUBOBJECT_TYPE_EXISTING_COLLECTION, &filtered_existing_a};
    ...
    if (SUCCEEDED(hr))
        hr = device5->CreateStateObject(&filtered_collection_desc_a, IID_PPV_ARGS(&filtered_raytracing_collection_a));
    ...
    D3D12_STATE_SUBOBJECT filtered_subobject_b = {D3D12_STATE_SUBOBJECT_TYPE_EXISTING_COLLECTION, &filtered_existing_b};
    ...
    if (SUCCEEDED(hr))
        hr = device5->CreateStateObject(&filtered_collection_desc_b, IID_PPV_ARGS(&filtered_raytracing_collection_b));
    const bool filtered_collections_created = filtered_raytracing_collection_a && filtered_raytracing_collection_b;
    ...
    D3D12_STATE_SUBOBJECT invalid_filtered_subobject = {D3D12_STATE_SUBOBJECT_TYPE_EXISTING_COLLECTION,
    ...
    const HRESULT invalid_filtered_hr =
        device5->CreateStateObject(&invalid_filtered_desc, IID_PPV_ARGS(&invalid_filtered_collection));
    ...
    existing_collections[0].pExistingCollection = filtered_raytracing_collection_a;
    existing_collections[1].pExistingCollection = filtered_raytracing_collection_b;
    D3D12_STATE_SUBOBJECT existing_collection_subobjects[2] = {
        {D3D12_STATE_SUBOBJECT_TYPE_EXISTING_COLLECTION, &existing_collections[0]},
        {D3D12_STATE_SUBOBJECT_TYPE_EXISTING_COLLECTION, &existing_collections[1]}};
    ...
    if (SUCCEEDED(hr))
        hr = device5->CreateStateObject(&pipeline_from_collection, IID_PPV_ARGS(&raytracing_state));
    const bool collection_pipeline_created = filtered_collections_created && raytracing_state != nullptr;
    safe_release(filtered_raytracing_collection_a);
    safe_release(filtered_raytracing_collection_b);
    if (SUCCEEDED(hr))
        hr = raytracing_state->QueryInterface(IID_PPV_ARGS(&raytracing_properties));

    D3D12_HIT_GROUP_DESC grown_hit_group = hit_group;
    grown_hit_group.HitGroupExport = L"grown_hit_group";
    D3D12_STATE_SUBOBJECT grown_subobject = {D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP, &grown_hit_group};
    ...
    if (SUCCEEDED(hr))
        hr = device7->AddToStateObject(&grown_desc, raytracing_state, IID_PPV_ARGS(&grown_raytracing_state));
    if (SUCCEEDED(hr))
    ...
    const void* miss_alias_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"miss_alias") : nullptr;
    const void* hit_group_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"hit_group") : nullptr;
    const void* grown_hit_group_identifier =
        SUCCEEDED(hr) ? grown_raytracing_properties->GetShaderIdentifier(L"grown_hit_group") : nullptr;
    const void* inherited_hit_group_identifier =
        SUCCEEDED(hr) ? grown_raytracing_properties->GetShaderIdentifier(L"hit_group") : nullptr;
    const void* callable_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"callable_shader") : nullptr;
    const void* callable_alias_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"callable_alias") : nullptr;
    const void* procedural_hit_group_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"procedural_hit_group") : nullptr;
    const void* repeated_raygen_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"raygen") : nullptr;
    const void* unknown_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"unknown_export") : nullptr;
    const void* filtered_out_identifier =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderIdentifier(L"closest_hit") : nullptr;
    const bool distinct_shader_identifiers =
        raygen_identifier && miss_identifier && hit_group_identifier && callable_identifier &&
        procedural_hit_group_identifier && std::memcmp(raygen_identifier, miss_identifier, 32) != 0 &&
        ...
        std::memcmp(hit_group_identifier, procedural_hit_group_identifier, 32) != 0;
    const bool stable_shader_identifiers = raygen_identifier && repeated_raygen_identifier &&
                                           std::memcmp(raygen_identifier, repeated_raygen_identifier, 32) == 0 &&
                                           unknown_identifier == nullptr;
    const bool renamed_export_identifiers = miss_alias_identifier && callable_alias_identifier && miss_identifier &&
                                            callable_identifier &&
    ...
    if (raygen_identifier)
        std::memcpy(&raygen_local_sampler_address, static_cast<const uint8_t*>(raygen_identifier) + 16,
                    sizeof(raygen_local_sampler_address));
    if (miss_identifier)
        std::memcpy(&miss_local_sampler_address, static_cast<const uint8_t*>(miss_identifier) + 16,
                    sizeof(miss_local_sampler_address));
    if (hit_group_identifier)
        std::memcpy(&hit_group_local_sampler_address, static_cast<const uint8_t*>(hit_group_identifier) + 16,
                    sizeof(hit_group_local_sampler_address));
    if (callable_identifier)
        std::memcpy(&callable_local_sampler_address, static_cast<const uint8_t*>(callable_identifier) + 16,
                    sizeof(callable_local_sampler_address));
    const bool shader_identifier_abi_layout =
        miss_alias_identifier && miss_identifier && callable_alias_identifier && callable_identifier &&
        raygen_local_sampler_address == 0 && miss_local_sampler_address == 0 && hit_group_local_sampler_address == 0 &&
        callable_local_sampler_address == 0 &&
        std::memcmp(static_cast<const uint8_t*>(miss_alias_identifier) + 24,
                    static_cast<const uint8_t*>(miss_identifier) + 24, sizeof(uint64_t)) != 0 &&
        std::memcmp(static_cast<const uint8_t*>(callable_alias_identifier) + 24,
                    static_cast<const uint8_t*>(callable_identifier) + 24, sizeof(uint64_t)) != 0;
    const bool collection_filtering_and_merge =
        collection_pipeline_created && filtered_out_identifier == nullptr && invalid_collection_export_rejected &&
        raygen_identifier && miss_identifier && hit_group_identifier && procedural_hit_group_identifier &&
        callable_identifier && miss_alias_identifier && callable_alias_identifier;
    const uint64_t raygen_stack_size =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderStackSize(L"raygen") : UINT64_C(0xffffffff);
    const uint64_t miss_stack_size =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderStackSize(L"miss_shader") : UINT64_C(0xffffffff);
    const uint64_t closest_hit_stack_size =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderStackSize(L"hit_group::closesthit") : UINT64_C(0xffffffff);
    const uint64_t any_hit_stack_size =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderStackSize(L"hit_group::anyhit") : UINT64_C(0xffffffff);
    const uint64_t callable_stack_size =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderStackSize(L"callable_shader") : UINT64_C(0xffffffff);
    const uint64_t hit_group_stack_size = SUCCEEDED(hr) ? raytracing_properties->GetShaderStackSize(L"hit_group") : 0;
    const uint64_t unknown_stack_size =
        SUCCEEDED(hr) ? raytracing_properties->GetShaderStackSize(L"unknown_export") : 0;
    const uint64_t initial_pipeline_stack_size = SUCCEEDED(hr) ? raytracing_properties->GetPipelineStackSize() : 0;
    if (SUCCEEDED(hr))
        raytracing_properties->SetPipelineStackSize(512);
    const uint64_t configured_pipeline_stack_size = SUCCEEDED(hr) ? raytracing_properties->GetPipelineStackSize() : 0;
    if (SUCCEEDED(hr))
        raytracing_properties->SetPipelineStackSize(UINT64_C(0xffffffff));
    const uint64_t rejected_pipeline_stack_size = SUCCEEDED(hr) ? raytracing_properties->GetPipelineStackSize() : 0;
    const bool stack_size_contract =
        raygen_stack_size > 0 && raygen_stack_size < UINT64_C(0xffffffff) && miss_stack_size > 0 &&
        miss_stack_size < UINT64_C(0xffffffff) && closest_hit_stack_size > 0 &&
        closest_hit_stack_size < UINT64_C(0xffffffff) && any_hit_stack_size > 0 &&
        any_hit_stack_size < UINT64_C(0xffffffff) && callable_stack_size > 0 &&
        callable_stack_size < UINT64_C(0xffffffff) && hit_group_stack_size == UINT64_C(0xffffffff) &&
        unknown_stack_size == UINT64_C(0xffffffff) && initial_pipeline_stack_size > 0 &&
        configured_pipeline_stack_size == 512 && rejected_pipeline_stack_size == configured_pipeline_stack_size;
    const bool add_to_state_object_created = grown_raytracing_state != nullptr;
    const bool grown_state_identifiers = grown_hit_group_identifier && inherited_hit_group_identifier &&
                                         hit_group_identifier &&
                                         std::memcmp(inherited_hit_group_identifier, hit_group_identifier, 32) == 0 &&
                                         std::memcmp(grown_hit_group_identifier, hit_group_identifier, 32) != 0;
    if (SUCCEEDED(hr) && (!distinct_shader_identifiers || !stable_shader_identifiers || !grown_state_identifiers))
        hr = E_FAIL;
    if (SUCCEEDED(hr) && (!renamed_export_identifiers || !shader_identifier_abi_layout ||
                          !collection_filtering_and_merge || !stack_size_contract))
        hr = E_FAIL;
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES upload_heap = heap_props(D3D12_HEAP_TYPE_UPLOAD);
        D3D12_RESOURCE_DESC table_desc = buffer_desc(640);
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &table_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
        ...
        if (SUCCEEDED(hr)) {
            ...
            std::memcpy(static_cast<uint8_t*>(mapped) + 64, miss_identifier, 32);
            std::memcpy(static_cast<uint8_t*>(mapped) + 160, miss_alias_identifier, 32);
            std::memcpy(static_cast<uint8_t*>(mapped) + 256, grown_hit_group_identifier, 32);
            std::memcpy(static_cast<uint8_t*>(mapped) + 352, procedural_hit_group_identifier, 32);
            std::memcpy(static_cast<uint8_t*>(mapped) + 448, callable_identifier, 32);
            std::memcpy(static_cast<uint8_t*>(mapped) + 544, callable_alias_identifier, 32);
            ...
            for (uint32_t record_offset : local_record_offsets)
                std::memcpy(static_cast<uint8_t*>(mapped) + record_offset + 32, &closest_hit_local_marker,
    ...
    const float initial_triangle[9] = {
        9.25f, -0.75f, 0.0f, 10.0f, 0.75f, 0.0f, 10.75f, -0.75f, 0.0f,
    };
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &index_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&indices));
        void* mapped = nullptr;
        if (SUCCEEDED(hr))
    ...
    geometry.Triangles.IndexBuffer = indices ? indices->GetGPUVirtualAddress() : 0;
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &as_desc,
    ...
    if (SUCCEEDED(hr)) {
        ...
        D3D12_RESOURCE_DESC scratch_desc =
            buffer_desc(std::max(prebuild.ScratchDataSizeInBytes, prebuild.UpdateScratchDataSizeInBytes));
        scratch_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &scratch_desc,
                                             D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&scratch));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC postbuild_desc = buffer_desc(256);
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &postbuild_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&postbuild));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC postbuild_desc = buffer_desc(256);
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &postbuild_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&clone_postbuild));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC postbuild_desc = buffer_desc(256);
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &buffer,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC buffer = buffer_desc(256);
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &buffer,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        D3D12_RESOURCE_DESC buffer = buffer_desc(256);
        hr =
            device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &buffer,
                                            D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&serialization_copy));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC buffer = buffer_desc(256);
    ...
    const D3D12_RAYTRACING_AABB aabb = {-0.5f, -0.5f, 0.0f, 0.5f, 0.5f, 1.0f};
    const D3D12_RAYTRACING_AABB initial_aabb = {9.5f, -0.5f, 0.0f, 10.5f, 0.5f, 1.0f};
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES upload_heap = heap_props(D3D12_HEAP_TYPE_UPLOAD);
        D3D12_RESOURCE_DESC aabb_desc = buffer_desc(sizeof(aabb));
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &aabb_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&aabbs));
        void* mapped = nullptr;
        if (SUCCEEDED(hr))
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &aabb_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&initial_aabbs));
        void* mapped = nullptr;
        if (SUCCEEDED(hr))
    ...
    aabb_geometry.Type = D3D12_RAYTRACING_GEOMETRY_TYPE_PROCEDURAL_PRIMITIVE_AABBS;
    aabb_geometry.Flags = D3D12_RAYTRACING_GEOMETRY_FLAG_NONE;
    aabb_geometry.AABBs.AABBCount = 1;
    aabb_geometry.AABBs.AABBs.StartAddress = initial_aabbs ? initial_aabbs->GetGPUVirtualAddress() : 0;
    aabb_geometry.AABBs.AABBs.StrideInBytes = sizeof(aabb);
    D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS aabb_inputs = {};
    aabb_inputs.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
    aabb_inputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE |
                        D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_UPDATE;
    ...
    if (SUCCEEDED(hr)) {
        device5->GetRaytracingAccelerationStructurePrebuildInfo(&aabb_inputs, &aabb_prebuild);
        if (!aabb_prebuild.ResultDataMaxSizeInBytes || !aabb_prebuild.ScratchDataSizeInBytes)
            hr = E_NOTIMPL;
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC as_desc = buffer_desc(aabb_prebuild.ResultDataMaxSizeInBytes);
        as_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &as_desc,
                                             D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC scratch_desc =
            buffer_desc(std::max(aabb_prebuild.ScratchDataSizeInBytes, aabb_prebuild.UpdateScratchDataSizeInBytes));
        scratch_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &scratch_desc,
                                             D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        D3D12_RESOURCE_DESC postbuild_desc = buffer_desc(256);
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &postbuild_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&aabb_postbuild));
    }

    constexpr UINT multi_geometry_count = 12;
    std::array<D3D12_RAYTRACING_GEOMETRY_DESC, multi_geometry_count> multi_geometry = {};
    std::array<const D3D12_RAYTRACING_GEOMETRY_DESC*, multi_geometry_count> multi_geometry_ptrs = {};
    ...
    multi_inputs.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
    multi_inputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE;
    ...
    if (SUCCEEDED(hr)) {
        device5->GetRaytracingAccelerationStructurePrebuildInfo(&multi_inputs, &multi_prebuild);
        if (!multi_prebuild.ResultDataMaxSizeInBytes || !multi_prebuild.ScratchDataSizeInBytes)
            hr = E_NOTIMPL;
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC as_desc = buffer_desc(multi_prebuild.ResultDataMaxSizeInBytes);
        as_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &as_desc,
                                             D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC scratch_desc = buffer_desc(multi_prebuild.ScratchDataSizeInBytes);
        scratch_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &scratch_desc,
                                             D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
    ...
    D3D12_RAYTRACING_GEOMETRY_DESC mixed_geometry[2] = {geometry, aabb_geometry};
    D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS mixed_inputs = {};
    mixed_inputs.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
    mixed_inputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PREFER_FAST_TRACE |
                         D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_ALLOW_UPDATE;
    mixed_inputs.NumDescs = 2;
    mixed_inputs.DescsLayout = D3D12_ELEMENTS_LAYOUT_ARRAY;
    mixed_inputs.pGeometryDescs = mixed_geometry;
    D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO mixed_prebuild = {};
    if (SUCCEEDED(hr)) {
        device5->GetRaytracingAccelerationStructurePrebuildInfo(&mixed_inputs, &mixed_prebuild);
        if (!mixed_prebuild.ResultDataMaxSizeInBytes || !mixed_prebuild.ScratchDataSizeInBytes)
            hr = E_NOTIMPL;
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC as_desc = buffer_desc(mixed_prebuild.ResultDataMaxSizeInBytes);
        as_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &as_desc,
                                             D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr,
                                             IID_PPV_ARGS(&mixed_geometry_acceleration_structure));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC scratch_desc =
            buffer_desc(std::max(mixed_prebuild.ScratchDataSizeInBytes, mixed_prebuild.UpdateScratchDataSizeInBytes));
        scratch_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &scratch_desc,
                                             D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
                                             IID_PPV_ARGS(&mixed_geometry_scratch));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC postbuild_desc = buffer_desc(256);
        hr = device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &postbuild_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                             IID_PPV_ARGS(&mixed_geometry_postbuild));
    }

    constexpr UINT top_level_instance_count = 12;
    std::array<D3D12_RAYTRACING_INSTANCE_DESC, top_level_instance_count> instance = {};
    ...
    instance[0].Flags = D3D12_RAYTRACING_INSTANCE_FLAG_TRIANGLE_CULL_DISABLE;
    instance[0].AccelerationStructure =
        deserialized_acceleration_structure ? deserialized_acceleration_structure->GetGPUVirtualAddress() : 0;
    ...
    instance[1].AccelerationStructure =
        aabb_acceleration_structure ? aabb_acceleration_structure->GetGPUVirtualAddress() : 0;
    ...
    instance[2].Flags = D3D12_RAYTRACING_INSTANCE_FLAG_TRIANGLE_CULL_DISABLE;
    instance[2].AccelerationStructure =
        multi_geometry_acceleration_structure ? multi_geometry_acceleration_structure->GetGPUVirtualAddress() : 0;
    // Exercise a mixed triangle/AABB BLAS through the same TLAS path.  The
    // Metal bridge flattens its tagged child BLAS records into this TLAS
    // rather than nesting a TLAS as a BLAS instance.
    instance[3].Transform[0][0] = 1.0f;
    instance[3].Transform[1][1] = 1.0f;
    instance[3].Transform[2][2] = 1.0f;
    instance[3].Transform[0][3] = 4.0f;
    instance[3].InstanceID = 10;
    instance[3].InstanceMask = 0x02;
    instance[3].AccelerationStructure =
        mixed_geometry_acceleration_structure ? mixed_geometry_acceleration_structure->GetGPUVirtualAddress() : 0;
    for (UINT i = 4; i < top_level_instance_count; i++) {
    ...
    std::array<D3D12_RAYTRACING_INSTANCE_DESC, top_level_instance_count> initial_instance = {};
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr = device->CreateCommittedResource(&upload_heap, D3D12_HEAP_FLAG_NONE, &instance_desc,
                                             D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
        ...
        if (SUCCEEDED(hr)) {
            std::memcpy(mapped, initial_instance.data(), sizeof(initial_instance));
    ...
    top_level_inputs.InstanceDescs = initial_instances ? initial_instances->GetGPUVirtualAddress() : 0;
    ...
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC tlas_desc = buffer_desc(top_level_prebuild.ResultDataMaxSizeInBytes);
        tlas_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &tlas_desc,
                                             D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        D3D12_RESOURCE_DESC scratch_desc = buffer_desc(
            std::max(top_level_prebuild.ScratchDataSizeInBytes, top_level_prebuild.UpdateScratchDataSizeInBytes));
        scratch_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &scratch_desc,
                                             D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        heap_desc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;
        hr = device->CreateDescriptorHeap(&heap_desc, IID_PPV_ARGS(&ray_query_heap));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES default_heap = heap_props(D3D12_HEAP_TYPE_DEFAULT);
        D3D12_RESOURCE_DESC output_desc = buffer_desc(256);
        output_desc.Flags = D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
        hr = device->CreateCommittedResource(&default_heap, D3D12_HEAP_FLAG_NONE, &output_desc,
                                             D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
    ...
    if (SUCCEEDED(hr)) {
        ...
        hr =
            device->CreateCommittedResource(&readback_heap, D3D12_HEAP_FLAG_NONE, &readback_desc,
                                            D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&ray_query_readback));
    }
    if (SUCCEEDED(hr)) {
        D3D12_HEAP_PROPERTIES indirect_readback_heap = heap_props(D3D12_HEAP_TYPE_READBACK);
        D3D12_RESOURCE_DESC indirect_readback_desc = buffer_desc(256);
        hr = device->CreateCommittedResource(&indirect_readback_heap, D3D12_HEAP_FLAG_NONE, &indirect_readback_desc,
                                             D3D12_RESOURCE_STATE_COPY_DEST, nullptr,
                                             IID_PPV_ARGS(&indirect_ray_readback));
    }
    if (SUCCEEDED(hr)) {
        const UINT increment = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
        D3D12_CPU_DESCRIPTOR_HANDLE cpu = ray_query_heap->GetCPUDescriptorHandleForHeapStart();
        D3D12_SHADER_RESOURCE_VIEW_DESC acceleration_srv = {};
        acceleration_srv.ViewDimension = D3D12_SRV_DIMENSION_RAYTRACING_ACCELERATION_STRUCTURE;
        acceleration_srv.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
        acceleration_srv.RaytracingAccelerationStructure.Location =
            deserialized_top_level_acceleration_structure->GetGPUVirtualAddress();
        ...
        device->CreateUnorderedAccessView(ray_query_output, nullptr, &output_uav, cpu);
        ...
        local_srv_desc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
        local_srv_desc.Buffer.NumElements = 64;
        local_srv_desc.Buffer.Flags = D3D12_BUFFER_SRV_FLAG_RAW;
        device->CreateShaderResourceView(closest_hit_local_srv, &local_srv_desc, cpu);
        ...
        local_texture_srv_desc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
        local_texture_srv_desc.Texture2D.MipLevels = 1;
        device->CreateShaderResourceView(closest_hit_local_texture, &local_texture_srv_desc, cpu);
        ...
        device->CreateUnorderedAccessView(closest_hit_local_uav, nullptr, &local_uav_desc, cpu);

        cpu.ptr += increment;
        D3D12_CONSTANT_BUFFER_VIEW_DESC local_cbv_desc = {};
        local_cbv_desc.BufferLocation = closest_hit_local_cbv->GetGPUVirtualAddress();
        local_cbv_desc.SizeInBytes = 256;
        device->CreateConstantBufferView(&local_cbv_desc, cpu);
        D3D12_GPU_DESCRIPTOR_HANDLE local_srv_handle = ray_query_heap->GetGPUDescriptorHandleForHeapStart();
        ...
        D3D12_GPU_DESCRIPTOR_HANDLE local_sampler_handle = local_sampler_heap->GetGPUDescriptorHandleForHeapStart();
        void* table_mapped = nullptr;
        if (raygen_shader_table && SUCCEEDED(raygen_shader_table->Map(0, nullptr, &table_mapped)) && table_mapped) {
            D3D12_GPU_DESCRIPTOR_HANDLE local_cbv_handle = ray_query_heap->GetGPUDescriptorHandleForHeapStart();
            local_cbv_handle.ptr += increment * 5;
            const uint32_t table_offsets[] = {64, 160, 256, 448, 544};
            for (uint32_t table_offset : table_offsets) {
                std::memcpy(static_cast<uint8_t*>(table_mapped) + table_offset + 40, &local_srv_handle,
                            sizeof(local_srv_handle));
                std::memcpy(static_cast<uint8_t*>(table_mapped) + table_offset + 48, &local_uav_handle,
                            sizeof(local_uav_handle));
                std::memcpy(static_cast<uint8_t*>(table_mapped) + table_offset + 56, &local_cbv_handle,
                            sizeof(local_cbv_handle));
                std::memcpy(static_cast<uint8_t*>(table_mapped) + table_offset + 64, &local_sampler_handle,
    ...
    if (SUCCEEDED(hr)) {
        ...
        build.DestAccelerationStructureData = acceleration_structure->GetGPUVirtualAddress();
        build.ScratchAccelerationStructureData = scratch->GetGPUVirtualAddress();
        build.Inputs = inputs;
        D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_DESC
        inline_serialization_post = {};
        inline_serialization_post.DestBuffer = serialization_postbuild->GetGPUVirtualAddress();
        inline_serialization_post.InfoType = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_SERIALIZATION;
        list4->BuildRaytracingAccelerationStructure(&build, 1, &inline_serialization_post);

        D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_DESC post = {};
        post.DestBuffer = postbuild->GetGPUVirtualAddress();
        post.InfoType = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_CURRENT_SIZE;
        D3D12_GPU_VIRTUAL_ADDRESS source = acceleration_structure->GetGPUVirtualAddress();
        list4->EmitRaytracingAccelerationStructurePostbuildInfo(&post, 1, &source);
        list4->CopyRaytracingAccelerationStructure(cloned_acceleration_structure->GetGPUVirtualAddress(),
        ...
        update_geometry.Triangles.VertexBuffer.StartAddress = updated_vertices->GetGPUVirtualAddress();
        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS update_inputs = inputs;
        update_inputs.Flags = static_cast<D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS>(
        ...
        update_build.DestAccelerationStructureData = cloned_acceleration_structure->GetGPUVirtualAddress();
        update_build.SourceAccelerationStructureData = cloned_acceleration_structure->GetGPUVirtualAddress();
        update_build.ScratchAccelerationStructureData = scratch->GetGPUVirtualAddress();
        update_build.Inputs = update_inputs;
        list4->BuildRaytracingAccelerationStructure(&update_build, 0, nullptr);
        D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_DESC
        compacted_post = {};
        compacted_post.DestBuffer = compacted_size_postbuild->GetGPUVirtualAddress();
        compacted_post.InfoType = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_COMPACTED_SIZE;
        source = cloned_acceleration_structure->GetGPUVirtualAddress();
        list4->EmitRaytracingAccelerationStructurePostbuildInfo(&compacted_post, 1, &source);
        list4->CopyRaytracingAccelerationStructure(compacted_acceleration_structure->GetGPUVirtualAddress(),
                                                   cloned_acceleration_structure->GetGPUVirtualAddress(),
                                                   D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_COMPACT);
        list4->CopyRaytracingAccelerationStructure(serialization_buffer->GetGPUVirtualAddress(),
                                                   compacted_acceleration_structure->GetGPUVirtualAddress(),
                                                   D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_SERIALIZE);
        list4->CopyBufferRegion(serialization_copy, 0, serialization_buffer, 0, 256);
        list4->CopyRaytracingAccelerationStructure(deserialized_acceleration_structure->GetGPUVirtualAddress(),
                                                   serialization_copy->GetGPUVirtualAddress(),
                                                   D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_DESERIALIZE);
        list4->CopyBufferRegion(serialization_readback, 0, serialization_copy, 0, 256);

        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC aabb_build = {};
        aabb_build.DestAccelerationStructureData = aabb_acceleration_structure->GetGPUVirtualAddress();
        aabb_build.ScratchAccelerationStructureData = aabb_scratch->GetGPUVirtualAddress();
        ...
        aabb_update_geometry.AABBs.AABBs.StartAddress = aabbs->GetGPUVirtualAddress();
        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC aabb_update = aabb_build;
        aabb_update.SourceAccelerationStructureData = aabb_acceleration_structure->GetGPUVirtualAddress();
        aabb_update.Inputs.Flags = static_cast<D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS>(
            aabb_inputs.Flags | D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE);
        ...
        multi_build.DestAccelerationStructureData = multi_geometry_acceleration_structure->GetGPUVirtualAddress();
        multi_build.ScratchAccelerationStructureData = multi_geometry_scratch->GetGPUVirtualAddress();
        ...
        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC mixed_build = {};
        mixed_build.DestAccelerationStructureData = mixed_geometry_acceleration_structure->GetGPUVirtualAddress();
        mixed_build.ScratchAccelerationStructureData = mixed_geometry_scratch->GetGPUVirtualAddress();
        mixed_build.Inputs = mixed_inputs;
        list4->BuildRaytracingAccelerationStructure(&mixed_build, 0, nullptr);
        post.DestBuffer = mixed_geometry_postbuild->GetGPUVirtualAddress();
        source = mixed_geometry_acceleration_structure->GetGPUVirtualAddress();
        list4->EmitRaytracingAccelerationStructurePostbuildInfo(&post, 1, &source);
        D3D12_RAYTRACING_GEOMETRY_DESC mixed_update_geometry[2] = {update_geometry, aabb_geometry};
        mixed_update_geometry[1].AABBs.AABBs.StartAddress = aabbs->GetGPUVirtualAddress();
        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_INPUTS mixed_update_inputs = mixed_inputs;
        mixed_update_inputs.Flags = static_cast<D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS>(
            mixed_inputs.Flags | D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE);
        mixed_update_inputs.pGeometryDescs = mixed_update_geometry;
        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC mixed_update = mixed_build;
        mixed_update.SourceAccelerationStructureData = mixed_geometry_acceleration_structure->GetGPUVirtualAddress();
        mixed_update.Inputs = mixed_update_inputs;
        list4->BuildRaytracingAccelerationStructure(&mixed_update, 0, nullptr);
        post.DestBuffer = mixed_geometry_postbuild->GetGPUVirtualAddress();
        source = mixed_geometry_acceleration_structure->GetGPUVirtualAddress();
        list4->EmitRaytracingAccelerationStructurePostbuildInfo(&post, 1, &source);

        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC top_level_build = {};
        top_level_build.DestAccelerationStructureData = top_level_acceleration_structure->GetGPUVirtualAddress();
        ...
        list4->BuildRaytracingAccelerationStructure(&top_level_build, 0, nullptr);
        D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC
        top_level_update = top_level_build;
        top_level_update.SourceAccelerationStructureData = top_level_acceleration_structure->GetGPUVirtualAddress();
        top_level_update.Inputs.InstanceDescs = instances->GetGPUVirtualAddress();
        top_level_update.Inputs.Flags = static_cast<D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS>(
            top_level_inputs.Flags | D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_PERFORM_UPDATE);
        D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_DESC
        top_level_serialization_post = {};
        top_level_serialization_post.DestBuffer = top_level_serialization_postbuild->GetGPUVirtualAddress();
        top_level_serialization_post.InfoType = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_POSTBUILD_INFO_SERIALIZATION;
        list4->BuildRaytracingAccelerationStructure(&top_level_update, 1, &top_level_serialization_post);
        post.DestBuffer = top_level_postbuild->GetGPUVirtualAddress();
        source = top_level_acceleration_structure->GetGPUVirtualAddress();
        list4->EmitRaytracingAccelerationStructurePostbuildInfo(&post, 1, &source);
        list4->CopyRaytracingAccelerationStructure(top_level_serialization_buffer->GetGPUVirtualAddress(),
                                                   top_level_acceleration_structure->GetGPUVirtualAddress(),
                                                   D3D12_RAYTRACING_ACCELERATION_STRUCTURE_COPY_MODE_SERIALIZE);
        list4->CopyBufferRegion(top_level_serialization_readback, 0, top_level_serialization_buffer, 0, 256);
        ...
        if (SUCCEEDED(hr)) {
            ...
            safe_release(mixed_geometry_acceleration_structure);
        ...
        if (SUCCEEDED(hr)) {
            ...
            list4->ClearRenderTargetView(local_texture_rtv, clear_color, 0, nullptr);
            D3D12_RESOURCE_BARRIER local_texture_barrier =
                transition_barrier(closest_hit_local_texture, D3D12_RESOURCE_STATE_RENDER_TARGET,
                                   D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        ...
        if (SUCCEEDED(hr))
            list4->SetComputeRootDescriptorTable(0, ray_query_heap->GetGPUDescriptorHandleForHeapStart());
        if (SUCCEEDED(hr))
            list4->Dispatch(1, 1, 1);
        if (SUCCEEDED(hr))
            list4->SetPipelineState1(grown_raytracing_state);
        D3D12_DISPATCH_RAYS_DESC dispatch_rays = {};
        dispatch_rays.RayGenerationShaderRecord.StartAddress = raygen_shader_table->GetGPUVirtualAddress();
        dispatch_rays.RayGenerationShaderRecord.SizeInBytes = 32;
        dispatch_rays.MissShaderTable.StartAddress = raygen_shader_table->GetGPUVirtualAddress() + 64;
        dispatch_rays.MissShaderTable.SizeInBytes = 192;
        dispatch_rays.MissShaderTable.StrideInBytes = 96;
        dispatch_rays.HitGroupTable.StartAddress = raygen_shader_table->GetGPUVirtualAddress() + 256;
        dispatch_rays.HitGroupTable.SizeInBytes = 192;
        dispatch_rays.HitGroupTable.StrideInBytes = 96;
        dispatch_rays.CallableShaderTable.StartAddress = raygen_shader_table->GetGPUVirtualAddress() + 448;
        dispatch_rays.CallableShaderTable.SizeInBytes = 192;
        dispatch_rays.CallableShaderTable.StrideInBytes = 96;
        dispatch_rays.Width = 5;
        ...
        D3D12_CPU_DESCRIPTOR_HANDLE output_cpu_handle = ray_query_heap->GetCPUDescriptorHandleForHeapStart();
        output_cpu_handle.ptr += descriptor_increment;
        D3D12_GPU_DESCRIPTOR_HANDLE output_gpu_handle = ray_query_heap->GetGPUDescriptorHandleForHeapStart();
        output_gpu_handle.ptr += descriptor_increment;
        list4->ClearUnorderedAccessViewUint(output_gpu_handle, output_cpu_handle, ray_query_output, clear_values, 0,
                                            nullptr);
        dispatch_rays.Width = 5;
        constexpr UINT64 indirect_argument_offset = 16;
        D3D12_INDIRECT_ARGUMENT_DESC indirect_ray_argument = {};
        indirect_ray_argument.Type = D3D12_INDIRECT_ARGUMENT_TYPE_DISPATCH_RAYS;
        ...
        indirect_ray_signature_hr = device->CreateCommandSignature(&indirect_ray_signature_desc, nullptr,
        ...
        if (SUCCEEDED(indirect_ray_signature_hr)) {
            ...
            D3D12_RESOURCE_DESC indirect_desc = buffer_desc(indirect_argument_offset + sizeof(dispatch_rays));
            indirect_ray_args_hr = device->CreateCommittedResource(&upload_properties, D3D12_HEAP_FLAG_NONE,
                                                                   &indirect_desc, D3D12_RESOURCE_STATE_GENERIC_READ,
                                                                   nullptr, IID_PPV_ARGS(&indirect_ray_args));
        }
        if (SUCCEEDED(indirect_ray_args_hr) && indirect_ray_args) {
            void* indirect_mapped = nullptr;
            indirect_ray_args_hr = indirect_ray_args->Map(0, nullptr, &indirect_mapped);
            if (SUCCEEDED(indirect_ray_args_hr) && indirect_mapped) {
                std::memcpy(static_cast<uint8_t*>(indirect_mapped) + indirect_argument_offset, &dispatch_rays,
                            sizeof(dispatch_rays));
                indirect_ray_args->Unmap(0, nullptr);
                list4->ExecuteIndirect(indirect_ray_signature, 1, indirect_ray_args, indirect_argument_offset, nullptr,
                                       0);
                indirect_ray_dispatch_recorded = true;
            }
        }
        D3D12_RESOURCE_BARRIER local_uav_barrier = transition_barrier(
            closest_hit_local_uav, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
        list4->ResourceBarrier(1, &local_uav_barrier);
        list4->CopyBufferRegion(closest_hit_local_uav_readback, 0, closest_hit_local_uav, 0, sizeof(uint32_t));
    ...
    uint64_t mixed_geometry_current_size = 0;
    bool mixed_geometry_verified = false;
    ...
    std::array<D3D12_GPU_VIRTUAL_ADDRESS, top_level_instance_count> serialized_bottom_level_pointers = {};
    ...
    D3D12_DRIVER_MATCHING_IDENTIFIER_STATUS serialization_unknown_status =
        D3D12_DRIVER_MATCHING_IDENTIFIER_UNRECOGNIZED;
    D3D12_DRIVER_MATCHING_IDENTIFIER_STATUS serialization_type_status = D3D12_DRIVER_MATCHING_IDENTIFIER_UNRECOGNIZED;
    uint32_t ray_hit = 0;
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            std::memcpy(&clone_current_size, mapped, sizeof(clone_current_size));
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            std::memcpy(&top_level_current_size, mapped, sizeof(top_level_current_size));
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            std::memcpy(&aabb_current_size, mapped, sizeof(aabb_current_size));
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            std::memcpy(&top_level_serialization_info, mapped, sizeof(top_level_serialization_info));
            top_level_serialization_postbuild->Unmap(0, nullptr);
        }
    }
    if (SUCCEEDED(hr)) {
        void* mapped = nullptr;
        D3D12_RANGE range = {0, sizeof(top_level_serialization_header) + sizeof(serialized_bottom_level_pointers)};
        hr = top_level_serialization_readback->Map(0, &range, &mapped);
        if (SUCCEEDED(hr)) {
            std::memcpy(&top_level_serialization_header, mapped, sizeof(top_level_serialization_header));
            std::memcpy(serialized_bottom_level_pointers.data(),
                        static_cast<const uint8_t*>(mapped) + sizeof(top_level_serialization_header),
            ...
            for (UINT i = 0; i < top_level_instance_count; i++) {
                serialized_pointer_list_matches_instances &=
                    serialized_bottom_level_pointers[i] == instance[i].AccelerationStructure;
            }
        }
    }
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            std::memcpy(&multi_geometry_current_size, mapped, sizeof(multi_geometry_current_size));
            multi_geometry_postbuild->Unmap(0, nullptr);
        }
    }
    if (SUCCEEDED(hr)) {
        void* mapped = nullptr;
        D3D12_RANGE range = {0, sizeof(mixed_geometry_current_size)};
        hr = mixed_geometry_postbuild->Map(0, &range, &mapped);
        if (SUCCEEDED(hr)) {
            std::memcpy(&mixed_geometry_current_size, mapped, sizeof(mixed_geometry_current_size));
            mixed_geometry_postbuild->Unmap(0, nullptr);
        }
    }
    mixed_geometry_verified =
        mixed_geometry_current_size > 0 && mixed_geometry_current_size <= mixed_prebuild.ResultDataMaxSizeInBytes;
    if (SUCCEEDED(hr)) {
        void* mapped = nullptr;
        D3D12_RANGE range = {0, sizeof(serialization_info)};
        hr = serialization_postbuild->Map(0, &range, &mapped);
        if (SUCCEEDED(hr)) {
            std::memcpy(&serialization_info, mapped, sizeof(serialization_info));
            serialization_postbuild->Unmap(0, nullptr);
        }
    }
    if (SUCCEEDED(hr)) {
        void* mapped = nullptr;
        D3D12_RANGE range = {0, sizeof(serialization_header) + sizeof(serialization_magic)};
        hr = serialization_readback->Map(0, &range, &mapped);
        if (SUCCEEDED(hr)) {
            std::memcpy(&serialization_header, mapped, sizeof(serialization_header));
            std::memcpy(&serialization_magic, static_cast<const uint8_t*>(mapped) + sizeof(serialization_header),
                        sizeof(serialization_magic));
            serialization_readback->Unmap(0, nullptr);
            serialization_identifier_status =
                device5->CheckDriverMatchingIdentifier(D3D12_SERIALIZED_DATA_RAYTRACING_ACCELERATION_STRUCTURE,
                                                       &serialization_header.DriverMatchingIdentifier);
            D3D12_SERIALIZED_DATA_DRIVER_MATCHING_IDENTIFIER mismatch = serialization_header.DriverMatchingIdentifier;
            mismatch.DriverOpaqueVersioningData[0] ^= 0x5a;
            serialization_version_status = device5->CheckDriverMatchingIdentifier(
                D3D12_SERIALIZED_DATA_RAYTRACING_ACCELERATION_STRUCTURE, &mismatch);
            mismatch = serialization_header.DriverMatchingIdentifier;
            mismatch.DriverOpaqueGUID.Data1 ^= 0x5a5a5a5a;
            serialization_unknown_status = device5->CheckDriverMatchingIdentifier(
                D3D12_SERIALIZED_DATA_RAYTRACING_ACCELERATION_STRUCTURE, &mismatch);
            serialization_type_status = device5->CheckDriverMatchingIdentifier(
                static_cast<D3D12_SERIALIZED_DATA_TYPE>(1), &serialization_header.DriverMatchingIdentifier);
        }
    }
    if (SUCCEEDED(hr)) {
        void* mapped = nullptr;
        ...
        if (SUCCEEDED(hr)) {
            ...
            std::memcpy(&miss_value, static_cast<const uint8_t*>(mapped) + sizeof(ray_hit), sizeof(miss_value));
            std::memcpy(&closest_hit_value, static_cast<const uint8_t*>(mapped) + sizeof(ray_hit) + sizeof(miss_value),
                        sizeof(closest_hit_value));
            std::memcpy(&callable_value, static_cast<const uint8_t*>(mapped) + sizeof(uint32_t) * 4,
            ...
            std::memcpy(&mixed_hit_value, static_cast<const uint8_t*>(mapped) + sizeof(uint32_t) * 6,
                        sizeof(mixed_hit_value));
            std::memcpy(&mixed_aabb_hit_value, static_cast<const uint8_t*>(mapped) + sizeof(uint32_t) * 7,
                        sizeof(mixed_aabb_hit_value));
    ...
    if (SUCCEEDED(hr)) {
        ...
        if (SUCCEEDED(hr)) {
            std::memcpy(&local_root_uav_value, mapped, sizeof(local_root_uav_value));
    ...
    const bool verified =
        ...
        compacted_size > 0 && compacted_size <= current_size && aabb_current_size > 0 &&
        aabb_current_size <= aabb_prebuild.ResultDataMaxSizeInBytes && multi_geometry_current_size > 0 &&
        multi_geometry_current_size <= multi_prebuild.ResultDataMaxSizeInBytes && mixed_geometry_verified &&
        serialization_info.SerializedSizeInBytes == 256 &&
        serialization_info.NumBottomLevelAccelerationStructurePointers == 0 &&
        serialization_header.SerializedSizeInBytesIncludingHeader == serialization_info.SerializedSizeInBytes &&
        serialization_header.DeserializedSizeInBytes == current_size &&
        serialization_header.NumBottomLevelAccelerationStructurePointersAfterHeader == 0 &&
        serialization_identifier_status == D3D12_DRIVER_MATCHING_IDENTIFIER_COMPATIBLE_WITH_DEVICE &&
        serialization_version_status == D3D12_DRIVER_MATCHING_IDENTIFIER_INCOMPATIBLE_VERSION &&
        serialization_unknown_status == D3D12_DRIVER_MATCHING_IDENTIFIER_UNRECOGNIZED &&
        serialization_type_status == D3D12_DRIVER_MATCHING_IDENTIFIER_UNSUPPORTED_TYPE &&
        serialization_magic == 0x4d54534153455231ull && top_level_serialization_info.SerializedSizeInBytes == 256 &&
        top_level_serialization_info.NumBottomLevelAccelerationStructurePointers == top_level_instance_count &&
        top_level_serialization_header.NumBottomLevelAccelerationStructurePointersAfterHeader ==
    ...
    safe_release(multi_geometry_acceleration_structure);
    safe_release(mixed_geometry_postbuild);
    safe_release(mixed_geometry_scratch);
    ...
    return {
        verified, verified ? S_OK : hr,
        verified ? "Metal twelve-geometry indexed/non-indexed triangle BLAS, clone, shifted triangle/AABB/TLAS "
                   "updates, compact copy, copied serialization blob/deserialization/TLAS traversal, filtered merged "
                   "collection-derived pipeline, local resource/sampler/static-sampler descriptor records, "
                   "shader-stack-size and pipeline-stack-size contracts, grown hit-group/local-root record plus "
                   "indexed renamed miss/callable records, inline RayQuery, and recursive "
                   "raygen/miss/any-hit/closest-hit/procedural/callable direct-indirect ray dispatch passed"
                 : "DXR acceleration-structure, inline-ray, or raygen gate failed",
        "\"prebuild_result_bytes\":" + std::to_string(prebuild.ResultDataMaxSizeInBytes) +
            ",\"ray_query_pso_created\":true" +
            ",\"prebuild_scratch_bytes\":" + std::to_string(prebuild.ScratchDataSizeInBytes) +
            ",\"current_size_bytes\":" + std::to_string(current_size) +
            ",\"clone_current_size_bytes\":" + std::to_string(clone_current_size) +
            ",\"compacted_size_bytes\":" + std::to_string(compacted_size) + ",\"blas_update_geometry_shift_x\":10" +
            ",\"aabb_prebuild_result_bytes\":" + std::to_string(aabb_prebuild.ResultDataMaxSizeInBytes) +
            ",\"aabb_current_size_bytes\":" + std::to_string(aabb_current_size) +
            ",\"aabb_update_geometry_shift_x\":10" +
            ",\"multi_geometry_count\":" + std::to_string(multi_geometry_count) +
            ",\"multi_geometry_prebuild_result_bytes\":" + std::to_string(multi_prebuild.ResultDataMaxSizeInBytes) +
            ",\"multi_geometry_current_size_bytes\":" + std::to_string(multi_geometry_current_size) +
            ",\"mixed_geometry_prebuild_result_bytes\":" + std::to_string(mixed_prebuild.ResultDataMaxSizeInBytes) +
            ",\"mixed_geometry_current_size_bytes\":" + std::to_string(mixed_geometry_current_size) +
            ",\"mixed_geometry_verified\":" + (mixed_geometry_verified ? "true" : "false") +
            ",\"serialized_size_bytes\":" + std::to_string(serialization_info.SerializedSizeInBytes) +
            ",\"serialization_bottom_level_pointer_count\":" +
            std::to_string(serialization_info.NumBottomLevelAccelerationStructurePointers) +
            ",\"serialization_blob_magic\":" + std::to_string(serialization_magic) +
            ",\"serialization_header_deserialized_size_bytes\":" +
            std::to_string(serialization_header.DeserializedSizeInBytes) + ",\"serialization_identifier_status\":" +
            std::to_string(static_cast<unsigned>(serialization_identifier_status)) +
            ",\"serialization_version_status\":" + std::to_string(static_cast<unsigned>(serialization_version_status)) +
            ",\"serialization_unknown_status\":" + std::to_string(static_cast<unsigned>(serialization_unknown_status)) +
            ",\"serialization_type_status\":" + std::to_string(static_cast<unsigned>(serialization_type_status)) +
            ",\"serialization_blob_copy_deserialized\":true" +
            ",\"tlas_serialized_size_bytes\":" + std::to_string(top_level_serialization_info.SerializedSizeInBytes) +
            ",\"tlas_serialization_bottom_level_pointer_count\":" +
            std::to_string(top_level_serialization_info.NumBottomLevelAccelerationStructurePointers) +
            ",\"tlas_instance_count\":" + std::to_string(top_level_instance_count) +
            ",\"tlas_serialized_pointer_list_matches_instances\":true" + ",\"tlas_deserialized_for_traversal\":true" +
            ",\"source_acceleration_structures_released_before_traversal\":" +
            (source_acceleration_structures_released_before_traversal ? "true" : "false") +
            ",\"tlas_prebuild_result_bytes\":" + std::to_string(top_level_prebuild.ResultDataMaxSizeInBytes) +
            ",\"tlas_prebuild_scratch_bytes\":" + std::to_string(top_level_prebuild.ScratchDataSizeInBytes) +
            ",\"tlas_current_size_bytes\":" + std::to_string(top_level_current_size) +
            ",\"tlas_update_instance_shift_x\":10" + ",\"inline_ray_hit\":" + std::to_string(ray_hit) +
            ",\"miss_dispatch_value\":" + std::to_string(miss_value) + ",\"closest_hit_dispatch_value\":" +
            std::to_string(closest_hit_value) + ",\"callable_dispatch_value\":" + std::to_string(callable_value) +
            ",\"raygen_dispatch_value\":" + std::to_string(raygen_value) + ",\"procedural_hit_dispatch_value\":" +
            std::to_string(procedural_hit_value) + ",\"mixed_hit_dispatch_value\":" + std::to_string(mixed_hit_value) +
            ",\"mixed_aabb_hit_dispatch_value\":" + std::to_string(mixed_aabb_hit_value) +
            ",\"miss_identifier_nonnull\":" + (miss_identifier ? "true" : "false") +
            ...
            ",\"invalid_collection_export_rejected\":" + (invalid_collection_export_rejected ? "true" : "false") +
            ",\"grown_state_identifiers\":" + (grown_state_identifiers ? "true" : "false") +
            ",\"renamed_export_identifiers\":" + (renamed_export_identifiers ? "true" : "false") +
            ",\"shader_identifier_abi_layout\":" + (shader_identifier_abi_layout ? "true" : "false") +
            ",\"shader_identifier_local_sampler_address\":" + std::to_string(raygen_local_sampler_address) +
            ",\"local_descriptor_tables_written\":" + (local_descriptor_tables_written ? "true" : "false") +
            ",\"local_sampler_table_written\":" + (local_sampler_table_written ? "true" : "false") +
            ",\"local_static_sampler_written\":" + (local_static_sampler_written ? "true" : "false") +
            ",\"shader_stack_size_contract\":" + (stack_size_contract ? "true" : "false") + ",\"raygen_stack_size\":" +
            std::to_string(raygen_stack_size) + ",\"miss_stack_size\":" + std::to_string(miss_stack_size) +
            ",\"closest_hit_stack_size\":" + std::to_string(closest_hit_stack_size) + ",\"any_hit_stack_size\":" +
            std::to_string(any_hit_stack_size) + ",\"callable_stack_size\":" + std::to_string(callable_stack_size) +
            ",\"initial_pipeline_stack_size\":" + std::to_string(initial_pipeline_stack_size) +
            ",\"configured_pipeline_stack_size\":" + std::to_string(configured_pipeline_stack_size) +
            ",\"rejected_pipeline_stack_size\":" + std::to_string(rejected_pipeline_stack_size) +
            ",\"miss_shader_table_records\":2" + ",\"callable_shader_table_records\":2" +
            ",\"miss_shader_table_stride\":64" + ",\"callable_shader_table_stride\":64" +
            ",\"indirect_ray_signature_hr\":\"" + hr_hex(indirect_ray_signature_hr) + "\",\"indirect_ray_args_hr\":\"" +
