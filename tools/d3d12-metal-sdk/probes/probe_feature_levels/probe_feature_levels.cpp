...
static void configure_exported_sdk() {
  ...
  if (!version.empty())
    D3D12SDKVersion = static_cast<UINT>(
        std::strtoul(version.c_str(), nullptr, 10));
...
int main() {
  ...
  constexpr std::array<D3D_FEATURE_LEVEL, 5> required_levels = {
  ...
  for (size_t i = 0; i < required_levels.size(); ++i) {
    ...
    HRESULT hr = create_device
                     ? create_device(nullptr, required_levels[i],
  ...
  HRESULT invalid_level_hr = create_device
                                 ? create_device(
                                       ...
                                       IID_D3D12DeviceProbe,
                                       reinterpret_cast<void **>(&invalid_device))
                                 : E_NOINTERFACE;
  if (invalid_device)
...
#define CHECK_FEATURE(name, value)                                             \
  name##_hr = device->CheckFeatureSupport(D3D12_FEATURE_##value, &name,       \
                                           sizeof(name))
  if (device) {
...
#undef CHECK_FEATURE
  ...
  for (const auto &result : create_results)
    all_levels_create = all_levels_create && SUCCEEDED(result.hr) &&
                        result.object_returned;

  const D3D12_COMMAND_LIST_SUPPORT_FLAGS required_write_immediate =
      static_cast<D3D12_COMMAND_LIST_SUPPORT_FLAGS>(
  ...
  const bool fl12_2_caps =
      ...
      SUCCEEDED(options_hr) && options.OutputMergerLogicOp &&
      ...
      options.MaxGPUVirtualAddressBitsPerResource >= 40 &&
      SUCCEEDED(options1_hr) && options1.WaveOps && options1.Int64ShaderOps &&
      SUCCEEDED(options2_hr) && options2.DepthBoundsTestSupported &&
      SUCCEEDED(options3_hr) && options3.CopyQueueTimestampQueriesSupported &&
      options3.CastingFullyTypedFormatSupported &&
      (options3.WriteBufferImmediateSupportFlags & required_write_immediate) ==
          required_write_immediate &&
      SUCCEEDED(options5_hr) &&
      options5.RaytracingTier >= D3D12_RAYTRACING_TIER_1_1 &&
      SUCCEEDED(options6_hr) &&
      ...
      gpu_va.MaxGPUVirtualAddressBitsPerProcess >= 40;

  const bool sm67_caps =
      SUCCEEDED(shader_model_hr) &&
      ...
      options14.AdvancedTextureOpsSupported &&
      options14.WriteableMSAATexturesSupported;

  const bool invalid_level_rejected = FAILED(invalid_level_hr);
  const bool pass = create_device && all_levels_create && fl12_2_caps &&
                    sm67_caps && invalid_level_rejected;
  ...
  std::printf("  \"sdk\": {\"version\": %u, \"path\": \"%s\"},\n",
              D3D12SDKVersion, json_escape(D3D12SDKPath).c_str());
  ...
  for (size_t i = 0; i < create_results.size(); ++i) {
    ...
    std::printf("    {\"level\": \"%s\", \"hr\": \"0x%08lx\", \"object_returned\": %s}%s\n",
                feature_level_name(result.level),
                static_cast<unsigned long>(static_cast<uint32_t>(result.hr)),
                result.object_returned ? "true" : "false",
                i + 1 == create_results.size() ? "" : ",");
  }
  std::printf("  ],\n");
