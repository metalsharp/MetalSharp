...
void cs_texture_1d(uint3 id : SV_DispatchThreadID) {
  output[0] = uint(tex1.Load(int2(0, 0)).r * 255.0 + 0.5);
}

[numthreads(1,1,1)]
void cs_texture_1d_array(uint3 id : SV_DispatchThreadID) {
  output[0] = uint(tex1a.Load(int3(0, 1, 0)).r * 255.0 + 0.5);
}

[numthreads(1,1,1)]
void cs_texture_2d(uint3 id : SV_DispatchThreadID) {
  output[0] = uint(tex2.Load(int3(0, 0, 0)).r * 255.0 + 0.5);
}

[numthreads(1,1,1)]
void cs_texture_2d_array(uint3 id : SV_DispatchThreadID) {
  output[0] = uint(tex2a.Load(int4(0, 0, 1, 0)).r * 255.0 + 0.5);
}

[numthreads(1,1,1)]
void cs_texture_3d(uint3 id : SV_DispatchThreadID) {
  output[0] = uint(tex3.Load(int4(0, 0, 1, 0)).r * 255.0 + 0.5);
}
