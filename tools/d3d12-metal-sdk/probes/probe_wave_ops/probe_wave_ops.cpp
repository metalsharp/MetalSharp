...
[numthreads(32, 1, 1)]
void cs_active_prefix_bit_count(uint3 id : SV_DispatchThreadID, uint gi : SV_GroupIndex) {
  bool bit = (gi & 1u) == 0u;
  uint active = WaveActiveCountBits(bit);
...
[numthreads(32, 1, 1)]
void cs_wave_multi_prefix_bit_count(uint3 id : SV_DispatchThreadID, uint gi : SV_GroupIndex) {
  uint4 match = WaveMatch(gi & 3u);
