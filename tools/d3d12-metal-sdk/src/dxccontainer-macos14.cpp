...
static void clear_bytecode(ShaderBytecode &out) {
    if (out.owned && out.data) std::free(out.data);
    out.data = nullptr;
    out.size = 0;
    out.kind = 0;
    out.owned = 0;
...
static bool convert_dxbc(ShaderBytecode &in, ShaderBytecode &out) {
    ...
    if (FAILED(hr) || !converter) { dlclose(handle); return false; }
    void *data = nullptr;
    uint32_t size = 0;
...
static bool compile_hlsl(const std::string &source, const char *profile, const char *entry,
    ...
    storage.emplace_back(widen(entry ? entry : "main")); args.push_back(storage.back().c_str());
    storage.emplace_back(L"-T"); args.push_back(storage.back().c_str());
    storage.emplace_back(widen(profile ? profile : "ps_6_0")); args.push_back(storage.back().c_str());
    ...
    append(extra); append(extra2);
    IDxcResult *result = nullptr;
    HRESULT hr = compiler->Compile(&buffer, args.data(), static_cast<uint32_t>(args.size()), nullptr,
                                   IID_PPV_ARGS(&result));
    if (FAILED(hr) || !result) { compiler->Release(); utils->Release(); return false; }
    HRESULT status = E_FAIL;
    result->GetStatus(&status);
    IDxcBlob *blob = nullptr;
    if (SUCCEEDED(status)) result->GetOutput(DXC_OUT_OBJECT, IID_PPV_ARGS(&blob), nullptr);
    bool ok = blob && copy_blob(out, blob->GetBufferPointer(), blob->GetBufferSize(), 2);
    if (blob) blob->Release();
    result->Release(); compiler->Release(); utils->Release();
    return ok;
}
...
bool DXBCToDXILE(ShaderBytecode &input, ShaderBytecode &output) {
    ...
    if (input.kind == 2) return copy_blob(output, input.data, input.size, 2);
...
bool GetGlobalRootSignature(const char *data, const unsigned char **out, unsigned *size) {
    ...
    if (!part) { *out = nullptr; *size = 0; return false; }
...
extern "C" bool dxccontainer_hlsl_to_dxil(
    const std::string &source, const char *profile, const char *entry,
    const std::vector<std::vector<wchar_t>> &extra,
    const std::vector<std::vector<wchar_t>> &extra2,
    ShaderBytecode &output)
    __asm__("__ZN10DXCompiler10HLSLToDXILERKNSt3__112basic_stringIcNS0_11char_traitsIcEENS0_9allocatorIcEEEEPKcSA_RKNS0_6vectorINS1_IwNS2_IwEENS4_ISE_EEEESI_R14ShaderBytecode");
extern "C" bool dxccontainer_hlsl_to_dxil(
    const std::string &source, const char *profile, const char *entry,
    const std::vector<std::vector<wchar_t>> &extra,
    const std::vector<std::vector<wchar_t>> &extra2,
...
extern "C" bool dxccontainer_dxbc_to_dxil(ShaderBytecode &input, ShaderBytecode &output)
    __asm__("__ZN10DXCompiler10DXBCToDXILE14ShaderBytecodeRS0_");
extern "C" bool dxccontainer_dxbc_to_dxil(ShaderBytecode &input, ShaderBytecode &output) {
    return DXCompiler::DXBCToDXILE(input, output);
}

extern "C" bool dxccontainer_validate(const char *data, size_t size)
    __asm__("__ZN10DXCompiler20IsValidDxilContainerEPKcm");
extern "C" bool dxccontainer_validate(const char *data, size_t size) {
