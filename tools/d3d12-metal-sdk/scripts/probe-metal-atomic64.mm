...
static id<MTLLibrary> compile_source(id<MTLDevice> device, NSString *source,
                                     NSError **error) {
...
kernel void test(device atomic_ulong *values [[buffer(0)]],
  ...
  atomic_max_explicit(values + (tid & 3), ulong(5 + tid),
    ...
    id<MTLLibrary> device_add_library =
        compile_source(device, device_add_source, &device_add_error);
    id<MTLLibrary> threadgroup_add_library =
        compile_source(device, threadgroup_add_source, &threadgroup_add_error);
    id<MTLLibrary> device_max_library =
        compile_source(device, device_max_source, &device_max_error);
    ...
    if (device_max_library) {
      ...
      id<MTLComputePipelineState> pso = [device
          newComputePipelineStateWithFunction:
              [device_max_library newFunctionWithName:@"test"]
                                           error:&pso_error];
      pso_created = pso != nil;
      if (pso) {
        id<MTLBuffer> buffer =
            [device newBufferWithBytes:observed
    ...
    const bool full_atomic64_unavailable =
        !device_add_library && !threadgroup_add_library;
    const bool pass = full_atomic64_unavailable && device_max_library &&
                      pso_created && dispatch_completed && max_values_exact;
    ...
    std::printf("  \"device_atomic_max_values\": [%llu,%llu,%llu,%llu],\n",
                ...
                static_cast<unsigned long long>(observed[3]));
    std::printf("  \"full_d3d12_atomic64_available_natively\": false,\n");
    std::printf("  \"decision\": \"%s\"\n",
                pass ? "software emulation required"
                     : "unexpected Metal atomic64 surface; investigate");
    std::printf("}\n");
    return pass ? 0 : 1;
  }
}
