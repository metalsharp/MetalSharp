...
int main(int argc, char** argv) {
    if (argc != 5 && argc != 6) {
        std::fprintf(stderr, "usage: compile-procedural-raytracing dxil global-root-json "
    ...
    auto* root_desc = IRVersionedRootSignatureDescriptorCreateFromJSON(root_json.c_str());
    auto* root = root_desc ? IRRootSignatureCreateFromDescriptor(root_desc, &error) : nullptr;
    if (!root) {
        std::fprintf(stderr, "root-signature error %u\n", error ? IRErrorGetCode(error) : 999u);
        return 4;
    }

    auto* input = IRObjectCreateFromDXIL(bytes.data(), bytes.size(), IRBytecodeOwnershipCopy);
    ...
    const bool procedural = mode.rfind("procedural_", 0) == 0 || mode == "@procedural-wrapper";
    ...
    if (argc == 6) {
        ...
        local_root_desc = IRVersionedRootSignatureDescriptorCreateFromJSON(local_root_json.c_str());
        local_root = local_root_desc ? IRRootSignatureCreateFromDescriptor(local_root_desc, &error) : nullptr;
        if (!local_root) {
