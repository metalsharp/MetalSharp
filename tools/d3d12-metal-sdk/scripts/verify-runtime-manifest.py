...
def locate_artifact(runtime_dir: Path, name: str, wine_dir: Path | None = None) -> Path:
    candidates = [runtime_dir / name]
    if name.endswith(".dll"):
        candidates.append(runtime_dir / "x86_64-windows" / name)
    if name.endswith((".so", ".dylib")):
        candidates.append(runtime_dir / "x86_64-unix" / name)

    # ntdll.so is a Wine loader artifact, not a DXMT provider artifact.  Keep
    # it in the same manifest because the selected PE/Unix bridge cannot be
    # reproduced without the matching Wine Unix loader, but resolve it from
    # the explicitly selected Wine installation rather than copying it into
    # the DXMT runtime directory.  The explicit argument prevents a stale
    # global Wine installation from silently satisfying the provenance check.
    if wine_dir is not None:
        candidates.extend(
            (
                wine_dir / name,
                wine_dir / "x86_64-unix" / name,
                wine_dir / "lib" / "wine" / "x86_64-unix" / name,
            )
        )
...
def verify_manifest(
    ...
    for name, expected in artifacts.items():
        ...
        path = locate_artifact(runtime_dir, name, wine_dir)
...
def generate_manifest(
    ...
    if wine_dir is not None:
        manifest["loader"] = {
            ...
            "note": "ntdll.so is resolved from this explicitly selected Wine installation.",
        }
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Explicit selected Wine installation root for loader artifacts such as ntdll.so",
    )
    parser.add_argument(
        "--generate",
        action="store_true",
    ...
    if args.generate:
        return generate_manifest(runtime_dir, args.wine_dir, args.output, args.verbose)

    return verify_manifest(args.manifest, runtime_dir, args.wine_dir, args.verbose)
