...
static void emitFunctionPrologue(LowerContext &ctx) {
    ...
    if (ctx.shader.kind == DxilShaderKind::Node) {
        ...
        if (ctx.options.node_routing) os << kNodeRoutingMSL;
        ...
        if (ctx.options.node_routing) {
            os << "static inline uint m12_node_resolve_output(device char *context, uint metadata, uint index) {\n";
            os << "  auto c = m12_node_route_context(context);\n";
            os << "  if (c == nullptr && context != nullptr && (*reinterpret_cast<device const uint *>(context) >= 4u && *reinterpret_cast<device const uint *>(context) != 8u)) return 0u;\n";
            os << "  return c != nullptr ? m12_node_route_find(c, metadata, index) : m12_node_create_output_handle(metadata, index);\n}\n";
            os << "static inline uint m12_node_index_output(device char *context, uint handle, uint index) {\n";
            os << "  auto c = m12_node_route_context(context);\n";
            os << "  if (c == nullptr && context != nullptr && (*reinterpret_cast<device const uint *>(context) >= 4u && *reinterpret_cast<device const uint *>(context) != 8u)) return 0u;\n";
            os << "  return c != nullptr ? m12_node_route_index(c, handle, index) : (index == 0u ? handle : 0u);\n}\n";
            os << "static inline bool m12_node_valid_output(device char *context, uint handle) {\n";
            os << "  auto c = m12_node_route_context(context);\n";
            os << "  if (c == nullptr && context != nullptr && (*reinterpret_cast<device const uint *>(context) >= 4u && *reinterpret_cast<device const uint *>(context) != 8u)) return false;\n";
            os << "  return c != nullptr ? m12_node_route_valid(c, handle) : handle != 0u;\n}\n";
        }
        ...
        os << "  if (backing == nullptr || c == nullptr || (c->version != 2u && c->version != 3u && c->version != 4u && c->version != 5u && c->version != 6u && c->version != 7u && c->version != 8u && c->version != 9u)) return (output & 0xffffu) | ((count & 0xffu) << 16) | ((per_thread & 1u) << 24);\n";
        ...
        os << "  uint generation = (c->version == 6u || c->version == 7u || c->version == 9u) ? (*reinterpret_cast<device const uint *>(context + 56ul) << 8u) : 0u;\n";
        ...
        os << "  device const m12_node_input_context *c = context != nullptr ? reinterpret_cast<device const m12_node_input_context *>(context) : nullptr;\n";
        os << "  if (backing != nullptr && c != nullptr && (c->version == 2u || c->version == 3u || c->version == 4u || c->version == 5u || c->version == 6u || c->version == 7u || c->version == 8u || c->version == 9u) && (record & 0xc0000000u) == 0x40000000u) {\n";
        ...
        os << "  if (c->version == 8u || c->version == 9u) { ulong batch = max(ulong(c->batch_size), 1ul); ulong first = ulong(group) * batch; return first < ulong(c->count) ? uint(min(batch, ulong(c->count) - first)) : 0u; }\n";
        ...
        os << "  if (first >= ulong(c->count)) return 0u;\n";
        os << "  return uint(min(batch, ulong(c->count) - first));\n";
        os << "}\n";
        ...
        os << "  device const m12_node_input_context *c = context != nullptr ? reinterpret_cast<device const m12_node_input_context *>(context) : nullptr;\n";
        os << "  if ((record & 0xc0000000u) == 0x40000000u && c != nullptr && (c->version == 2u || c->version == 3u || c->version == 4u || c->version == 5u || c->version == 6u || c->version == 7u || c->version == 8u || c->version == 9u)) {\n";
        ...
        os << "    if (c->version == 8u || c->version == 9u) {\n";
        os << "      ulong logical = ulong(group) * max(ulong(c->batch_size), 1ul) + ulong(index);\n";
        os << "      if (logical >= ulong(c->count) || c->size > c->length || (logical != 0ul && c->stride > (c->length - c->size) / logical)) return nullptr;\n";
        os << "      return input + logical * c->stride;\n    }\n";
        os << "    if (c->version == 3u || c->version == 5u || c->version == 7u) {\n";
        os << "      ulong batch = c->batch_size != 0u ? ulong(c->batch_size) : 1ul;\n";
        os << "      ulong logical = batch == 1ul ? ulong(group) : ulong(group) * batch + ulong(index);\n";
        os << "      if (logical >= ulong(c->count) || c->stride < 4ul || c->length < 4ul || logical > (c->length - 4ul) / c->stride || backing == nullptr) return nullptr;\n";
    ...
    if (ctx.ray_generation) {
        ...
        if (ctx.options.conservative_rasterization) {
            ...
            os << "static inline bool m12_cons_point_in_tri(float2 p, float2 a, float2 b, float2 c) { float e=1.0e-5f; if (abs(m12_cons_cross(a,b,c)) <= e) return false; float s0=m12_cons_cross(a,b,p), s1=m12_cons_cross(b,c,p), s2=m12_cons_cross(c,a,p); return (s0 >= -e && s1 >= -e && s2 >= -e) || (s0 <= e && s1 <= e && s2 <= e); }\n";
...
static std::string translateDXIntrinsic(LowerContext &ctx, uint32_t intrinsic_id,
    ...
    case DXOP_GetRemainingRecursionLevels: {
        ...
        case DXOP_CreateNodeOutputHandle: {
            ...
            if (ctx.options.node_routing)
                return "m12_node_resolve_output(buf28, " +
                       std::to_string(metadata) + "u, " + std::to_string(array_index) + "u)";
        ...
        case DXOP_IndexNodeHandle: {
            ...
            if (ctx.options.node_routing) {
                const std::string index = numericArg(1, "");
                if (index.empty()) return reject_node("node array index must be scalar");
                return "m12_node_index_output(buf28, (uint)(" +
                       valueArg(0, "0u") + "), (uint)(" + index + "))";
            }
        ...
        case DXOP_NodeOutputIsValid:
            ...
            if (ctx.options.node_routing)
                return "m12_node_valid_output(buf28, (uint)(" +
                       valueArg(0, "0u") + "))";
